...
import threading
import sqlite3, os, io, csv, hashlib, re
from werkzeug.utils import secure_filename
from datetime import datetime, date, timedelta
import calendar

app = Flask(__name__)
app.secret_key = "RDHRMSPayrollSolution2024"
...
def inject_pending_counts():
    ...
    try:
        if session.get("user"):
            ...
            # Dept Head pending — for users with dept_head_approve perm, or admins (implicit, all departments)
            if role == "admin" or "dept_head_approve" in perms:
                u = conn.execute("SELECT id FROM users WHERE username=?",
                                 (session.get("user",""),)).fetchone()
                if u:
                    if role == "admin":
                        dept_head_pending_count = conn.execute("""
                            SELECT COUNT(*) FROM leave_requests
                            WHERE dept_head_status='Pending' AND status='Pending'""").fetchone()[0]
                    else:
            ...
            # Associate OT pending approvals
            if role == "admin" or "assoc_leave_approve" in perms or "leave_assoc" in perms:
                try:
                    pending_assoc_ot_count = conn.execute(
                        "SELECT COUNT(*) FROM associate_ot_leave_requests WHERE status='Pending'"
                    ).fetchone()[0]
                except Exception:
                    pending_assoc_ot_count = 0
...
def esic_round_up(amount):
    """
    ESIC statutory rounding rule:
    Any paisa from 0.01 to 0.99 is rounded UP to the next whole rupee.
    Example: 50.20 -> 51, 50.00 -> 50, 50.99 -> 51
    """
    import math
    if amount is None:
        return 0
    return int(math.ceil(amount - 1e-9))  # tiny epsilon avoids float artefacts (e.g. 50.0000001)
...
def init_db():
    ...
    # ── Extra Working Hours — Settings ───────────────────────
    # Standalone module (separate from OT Rate Master / OT Process).
    # Per-hour rate for each employee is derived from their salary:
    #   per_hour_rate = (salary_basis_amount / days_divisor / shift_working_hours) * multiplier
    # Extra working hours themselves come from attendance.ot_minutes,
    # which is already computed by calc_att() as (worked_minutes - shift_working_hours).
    c.execute("""CREATE TABLE IF NOT EXISTS extra_working_settings (
        id INTEGER PRIMARY KEY DEFAULT 1,
        salary_basis TEXT DEFAULT 'gross',
        days_divisor REAL DEFAULT 26,
        multiplier REAL DEFAULT 1.0,
        updated_on TEXT)""")
    try:
        c.execute("INSERT OR IGNORE INTO extra_working_settings (id, salary_basis, days_divisor, multiplier) VALUES (1,'gross',26,1.0)")
    except: pass

    # Extra Working — Month Lock (freezes editing once locked)
    c.execute("""CREATE TABLE IF NOT EXISTS extra_working_lock (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        month INTEGER NOT NULL, year INTEGER NOT NULL,
        is_locked INTEGER DEFAULT 0,
        locked_by TEXT, locked_on TEXT,
        UNIQUE(month, year))""")

    # Extra Working — Snapshot (frozen per-employee totals; feeds Payroll Insights once locked)
    c.execute("""CREATE TABLE IF NOT EXISTS extra_working_snapshot (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        emp_code TEXT NOT NULL, emp_name TEXT, department TEXT,
        month INTEGER NOT NULL, year INTEGER NOT NULL,
        extra_hours REAL DEFAULT 0, per_hr_rate REAL DEFAULT 0, extra_amount REAL DEFAULT 0,
    ...
    c.execute("""CREATE TABLE IF NOT EXISTS payroll_settings (
        id INTEGER PRIMARY KEY DEFAULT 1,
        company_name TEXT DEFAULT 'RD HRMS & Payroll Solution',
    ...
    c.execute("""CREATE TABLE IF NOT EXISTS email_settings (
        ...
        sender_name TEXT DEFAULT 'RD HRMS & Payroll Solution',
    ...
    # Auto Leave Earn feature removed — Leave Earn is now Admin/HR-triggered only (Earn Leave popup / Manual Earn Credit)
    c.execute("UPDATE leave_master_settings SET earn_auto_credit=0 WHERE id=1")

    # ── Leave Year-End Log (per-employee Carry Forward / Encash, duplicate-safe) ──
    c.execute("""CREATE TABLE IF NOT EXISTS leave_year_end_log (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        emp_code TEXT NOT NULL,
        year INTEGER NOT NULL,
        ...
        processed_by TEXT,
        processed_on TEXT,
        UNIQUE(emp_code, year, leave_type_code))""")
    try: c.execute("ALTER TABLE employees ADD COLUMN leave_encashment_amount REAL DEFAULT 0")
    except: pass
    ...
    # ── Increments: Designation Change Support ───────────────
    try: c.execute("ALTER TABLE increments ADD COLUMN old_designation TEXT DEFAULT ''")
    except: pass
    try: c.execute("ALTER TABLE increments ADD COLUMN new_designation TEXT DEFAULT ''")
    except: pass
    ...
    # Associate OT Leave Requests — dept head submits, HR/admin approves → authorizes OT
    c.execute("""CREATE TABLE IF NOT EXISTS associate_ot_leave_requests (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        emp_code TEXT NOT NULL,
        emp_name TEXT,
        department TEXT,
        ...
        approved_on TEXT,
        remarks TEXT)""")
    try: c.execute("ALTER TABLE associate_ot_leave_requests ADD COLUMN from_date TEXT")
    except: pass
    try: c.execute("ALTER TABLE associate_ot_leave_requests ADD COLUMN to_date TEXT")
    except: pass
    try: c.execute("ALTER TABLE associate_ot_leave_requests ADD COLUMN absent_date TEXT")
    except: pass
    try: c.execute("ALTER TABLE associate_ot_leave_requests ADD COLUMN working_days INTEGER DEFAULT 0")
    except: pass
    ...
    # Letter Generator — custom, admin-editable letter templates (format + text)
    c.execute("""CREATE TABLE IF NOT EXISTS letter_templates (
        ...
        signature_type TEXT DEFAULT 'hr',
        created_by TEXT,
        created_on TEXT,
        updated_on TEXT,
        active INTEGER DEFAULT 1)""")
    try: c.execute("ALTER TABLE letter_templates ADD COLUMN doc_code_prefix TEXT DEFAULT ''")
    except: pass
    try: c.execute("ALTER TABLE letter_templates ADD COLUMN signature_ids TEXT DEFAULT ''")
    except: pass
    try: c.execute("ALTER TABLE letter_templates ADD COLUMN format_no TEXT DEFAULT ''")
    except: pass
    try: c.execute("ALTER TABLE letter_templates ADD COLUMN margin_top REAL DEFAULT 15")
    except: pass
    try: c.execute("ALTER TABLE letter_templates ADD COLUMN margin_right REAL DEFAULT 18")
    except: pass
    try: c.execute("ALTER TABLE letter_templates ADD COLUMN margin_bottom REAL DEFAULT 15")
    except: pass
    try: c.execute("ALTER TABLE letter_templates ADD COLUMN margin_left REAL DEFAULT 18")
    except: pass
    try: c.execute("ALTER TABLE letter_templates ADD COLUMN page_size TEXT DEFAULT 'A4'")
    except: pass
    try: c.execute("ALTER TABLE letter_templates ADD COLUMN header_margin REAL DEFAULT 10")
    except: pass
    try: c.execute("ALTER TABLE letter_templates ADD COLUMN footer_margin REAL DEFAULT 10")
    except: pass

    # Named signatures — multiple, admin-managed (used by custom letter templates)
    c.execute("""CREATE TABLE IF NOT EXISTS letter_signatures (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
    ...
    # Generated letter documents — approval workflow.
    # A document only gets a doc_code (and can only be viewed/printed) once Approved.
    c.execute("""CREATE TABLE IF NOT EXISTS letter_documents (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        ...
        approved_on TEXT,
        reject_reason TEXT)""")
    try: c.execute("ALTER TABLE letter_documents ADD COLUMN format_no TEXT DEFAULT ''")
    except: pass
    try: c.execute("ALTER TABLE letter_documents ADD COLUMN margin_top REAL DEFAULT 15")
    except: pass
    try: c.execute("ALTER TABLE letter_documents ADD COLUMN margin_right REAL DEFAULT 18")
    except: pass
    try: c.execute("ALTER TABLE letter_documents ADD COLUMN margin_bottom REAL DEFAULT 15")
    except: pass
    try: c.execute("ALTER TABLE letter_documents ADD COLUMN margin_left REAL DEFAULT 18")
    except: pass
    try: c.execute("ALTER TABLE letter_documents ADD COLUMN letter_date TEXT DEFAULT ''")
    except: pass
    try: c.execute("ALTER TABLE letter_documents ADD COLUMN page_size TEXT DEFAULT 'A4'")
    except: pass
    try: c.execute("ALTER TABLE letter_documents ADD COLUMN header_margin REAL DEFAULT 10")
    except: pass
    try: c.execute("ALTER TABLE letter_documents ADD COLUMN footer_margin REAL DEFAULT 10")
    except: pass

    # Audit trail for edits made to a letter while it is still Pending approval.
    # A reason is mandatory for every edit; it is for internal record-keeping only
    # and is never printed on the letter itself.
    c.execute("""CREATE TABLE IF NOT EXISTS letter_document_edits (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
    ...
    # Reusable validation type per custom field key (Date / Number / Text), so once an
    # admin defines e.g. "notice_days" as Number, every template reuses that automatically.
    c.execute("""CREATE TABLE IF NOT EXISTS letter_field_types (
    ...
    # ── Login Page Branding — background image/video + headline ──
    for col, defval in [
        ("login_bg",           "BLOB"),
        ("login_bg_filename",  "TEXT"),
        ("login_bg_mimetype",  "TEXT"),
        ("login_bg_type",      "TEXT"),   # 'image' or 'video'
        ("login_headline",     "TEXT DEFAULT 'Welcome back — sign in to continue'"),
        ("login_subtext",      "TEXT DEFAULT ''"),
    ]:
        try: c.execute(f"ALTER TABLE company_settings ADD COLUMN {col} {defval}")
        except: pass
    ...
    # ── Shift-Master-driven attendance rules (replaces old hardcoded
    #    Staff/Associate category branching in calc_att). Fully editable
    #    per-shift from Shift Master going forward — nothing fixed in backend.
    #    min_present_minutes: worked_net below this = Absent (0 = rule off)
    #    half_day_late_cutoff: IN time after this (HH:MM) = Half Day instead of Late (blank = rule off)
    #    count_late_marks: whether late marks on this shift count toward the "2 lates = half day" rule
    try:
        c.execute("ALTER TABLE shifts ADD COLUMN min_present_minutes INTEGER DEFAULT 0")
        c.execute("ALTER TABLE shifts ADD COLUMN half_day_late_cutoff TEXT DEFAULT NULL")
    ...
    # Masters tables start empty — the operator's existing database already holds
    # its own Department / Designation / Location values; no sample data is pre-loaded
    # here to avoid duplicate/placeholder entries alongside real data.
    ...
    c.execute("""CREATE TABLE IF NOT EXISTS master_designations (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        name TEXT UNIQUE NOT NULL, is_active INTEGER DEFAULT 1)""")
    # Pre-load categories only — department/designation/location are left empty
    # (see note above; the operator's existing database already has its own values)
    ...
    # ── Weekly Off Module: dated assignment history ──────────────────
    c.execute("""CREATE TABLE IF NOT EXISTS weekly_off_history (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        emp_code TEXT NOT NULL,
    ...
    try: c.execute("CREATE INDEX IF NOT EXISTS idx_wo_hist_emp ON weekly_off_history(emp_code, from_date)")
    except: pass
    # One-time seed: preserve each employee's existing weekly_off value as an
    # open-ended assignment starting from a genesis date, so past/current
    # computations remain unchanged until an admin makes a new dated change
    # via the Weekly Off module (at which point this seed row gets closed).
    try:
        _seeded = {r["emp_code"] for r in c.execute("SELECT DISTINCT emp_code FROM weekly_off_history").fetchall()}
        _all_emps = c.execute("SELECT emp_code, COALESCE(weekly_off,'Sunday') as wo FROM employees").fetchall()
        for _e in _all_emps:
            if _e["emp_code"] not in _seeded:
                c.execute("""INSERT INTO weekly_off_history (emp_code, weekly_off_day, from_date, to_date, created_by)
                             VALUES (?,?,?,NULL,?)""",
                          (_e["emp_code"], _e["wo"] or "Sunday", "2000-01-01", "system_migration"))
    except: pass
    ...
    try: c.execute("ALTER TABLE leave_earn_log ADD COLUMN remarks TEXT")
    except: pass
    ...
    # Pre-load RD HRMS & Payroll Solution company shifts (eSSL exact fields)
    ...
    # ── One-time backfill: shift-master attendance rules ─────────
    # (min_present_minutes / half_day_late_cutoff / count_late_marks)
    # Preserves old hardcoded Staff/Associate attendance behavior explicitly
    # per-shift at migration time — fully editable from Shift Master from here on.
    # Tracked so it runs exactly once, regardless of fresh install vs upgrade,
    # and never re-overwrites values an admin customizes afterward.
    conn.execute("""CREATE TABLE IF NOT EXISTS _schema_migrations (
        name TEXT PRIMARY KEY, applied_on TEXT)""")
    if not conn.execute("SELECT 1 FROM _schema_migrations WHERE name='shift_category_rules_backfill'").fetchone():
        conn.execute("UPDATE shifts SET min_present_minutes=180 WHERE category='Associate'")
        conn.execute("UPDATE shifts SET half_day_late_cutoff='12:30' WHERE category='Staff'")
...
def etimetrack_pair_punches(all_punches_sorted, shifts_list, category="Associate",
    ...
    #   Max gap: 23 hrs (flat — shift-based pairing handles normal cases;
    #   this only applies to the rare no-shift-assigned fallback)
    #
    ...
    records      = []
    i            = 0
    max_gap_min  = 23 * 60  # flat default — no-shift fallback pairing only
...
_WO_NUM_MAP = {v: k for k, v in _WO_DAY_MAP.items()}
...
def get_emp_weekly_off_num(emp_code, conn=None, on_date=None):
    """Return weekday number (0=Mon..6=Sun) for employee's weekly off day.

    Respects dated Weekly Off assignments from the weekly_off_history table:
    - Looks up the most recent assignment whose [from_date, to_date] range
      covers `on_date` (defaults to today if not given).
    - If no assignment covers that date, the default is Sunday (6).
    This makes the weekly off day-specific/historical: changing an employee's
    weekly off mid-month only affects dates from the change onward; past
    dates keep whatever was in effect at that time.
    """
    close = False
    if conn is None:
    ...
    try:
        ...
        elif hasattr(on_date, "isoformat"):
            d_str = on_date.isoformat()
        else:
            d_str = str(on_date)[:10]
        row = conn.execute("""SELECT weekly_off_day FROM weekly_off_history
                               WHERE emp_code=? AND from_date<=? AND (to_date IS NULL OR to_date>=?)
                               ORDER BY from_date DESC LIMIT 1""",
                            (emp_code, d_str, d_str)).fetchone()
        if row and row["weekly_off_day"]:
            return _WO_DAY_MAP.get(row["weekly_off_day"], 6)
        return 6  # default Sunday when no dated assignment covers this date
    except: return 6
    finally:
        if close: conn.close()

def sync_weekly_off_column(conn=None):
    """Refresh employees.weekly_off (today's effective weekly off) from the
    weekly_off_history table for all employees. Many dashboards/exports/
    filters read the flat employees.weekly_off column directly for "today"
    status, so this keeps that column in sync with dated assignments —
    including auto-reverting to Sunday once a temporary range expires.
    Called whenever the Weekly Off module is opened or an assignment changes.
    """
    close = False
    if conn is None:
        conn = get_db(); close = True
    try:
        ...
        emps = conn.execute("SELECT emp_code, weekly_off FROM employees").fetchall()
        for e in emps:
            wo_num = get_emp_weekly_off_num(e["emp_code"], conn, on_date=today_str)
            wo_day = _WO_NUM_MAP.get(wo_num, "Sunday")
            if e["weekly_off"] != wo_day:
                conn.execute("UPDATE employees SET weekly_off=? WHERE emp_code=?", (wo_day, e["emp_code"]))
        conn.commit()
    except: pass
    finally:
        if close: conn.close()
...
def is_weekly_off_date(d, emp_code, conn=None):
    ...
    try:
        ...
        return weekday == get_emp_weekly_off_num(emp_code, conn, on_date=d)
    except: return False
...
def calc_att(emp_code, in_t, out_t, category, shift=None, status_override=None):
    ...
    if shift:
        ...
        # No shift assigned — use standard neutral default
        # (rare edge case; employee should appear on "Missing Shift" list for HR to fix)
        sh_start   = 9*60+0
        sh_end     = 17*60+30
        is_next_day= 0
    ...
    # ── Below minimum present threshold → Absent ─────────────────
    # Shift-configurable via Shift Master (min_present_minutes = 0 disables this rule).
    min_present = int(shift.get("min_present_minutes") or 0) if shift else 0
    if min_present > 0 and worked_net < min_present and worked_net > 0:
        r["status"] = "Absent"
        r["working_minutes"] = 0
        r["ot_minutes"] = 0
        return r
...
def get_wd(year, month, category, weekly_off="Sunday"):
    """
    Calculate working days for a given month based on employee's weekly off day.
    Formula: Total month days - Number of times the weekly-off day appears in that month.
    This is always employee-specific — monthly_working_days manual override is removed.
    """
    _wo_map = {"Monday":0,"Tuesday":1,"Wednesday":2,"Thursday":3,"Friday":4,"Saturday":5,"Sunday":6}
    wo_weekday = _wo_map.get(weekly_off, 6)  # default Sunday=6
    total_days = calendar.monthrange(year, month)[1]
    wo_count = sum(
        1 for d in range(1, total_days + 1)
        if date(year, month, d).weekday() == wo_weekday
    )
    return total_days - wo_count
...
def save_att_row(conn, emp_code, att_date, in_t, out_t, category, status="Present", status_override=None, pre_shift=None, late_count_map=None):
    ...
    if conn is not None:
        ...
        # ── Half Day Leave check: agar approved half day leave hai toh late = 0 ──
        # 1st half leave pe employee 2nd half mein aata hai → late nahi hai
        try:
            _hd_leave = conn.execute("""
                SELECT id FROM leave_requests
                WHERE emp_code=? AND is_half_day=1 AND status='Approved'
                  AND from_date <= ? AND to_date >= ?
            """, (emp_code, att_date, att_date)).fetchone()
            if _hd_leave:
                c["late_minutes"] = 0  # Half day leave approved → late mark nahi
        except: pass
...
def get_leave_fy_year(conn, on_date=None):
    """Return the Leave Financial-Year 'year' bucket (default Apr-Mar) for a given date.
    e.g. any date in Apr-2026..Mar-2027 returns 2026 (matches existing leave_balance.year convention)."""
    ...
    try:
        ...
        fy_start = 4
    return on_date.year if on_date.month >= fy_start else on_date.year - 1

def get_leave_earn_caps(conn):
    """Yearly leave-earn caps per Financial Year. Default EL=16 + CL=6 = 22 total/employee/year.
    Sourced from Payroll Settings (el_per_year / cl_per_year) so it stays admin-configurable."""
    ps = get_payroll_settings(conn)
    el_cap = float(ps.get("el_per_year", 16) or 16)
    cl_cap = float(ps.get("cl_per_year", 6) or 6)
    return el_cap, cl_cap

def leave_earn_room(conn, emp_code, year, code, cap):
    """Remaining room (days) that can still be earned this Financial Year for a given
    leave code ('el'/'cl') before hitting the yearly cap. Never allows earn beyond the cap."""
    try:
        row = conn.execute(f"SELECT {code}_allotted FROM leave_balance WHERE emp_code=? AND year=?",
                            (emp_code, year)).fetchone()
        current = float(row[0] or 0) if row else 0.0
    except Exception:
        current = 0.0
    return max(0.0, round(cap - current, 4))
...
def calc_salary_emp(emp_code, month, year, preview=False, force=False):
    ...
    try:
        ...
        cat = emp["category"]
        # Get employee's weekly off day and calculate working days from it
        _wo_row = conn.execute("SELECT weekly_off FROM employees WHERE emp_code=?", (emp_code,)).fetchone()
        emp_weekly_off_wd = (_wo_row["weekly_off"] if _wo_row and _wo_row["weekly_off"] else "Sunday")
        wd  = get_wd(year, month, cat, weekly_off=emp_weekly_off_wd)  # employee-specific working days
        ...
        for day in range(1, total_cal_days + 1):
            ...
            if st == "WO":
                ...
                # Approved Paid Leave — but NEVER count a weekly-off day as paid leave.
                # If leave spans across the employee's WO day, that day is simply WO (unpaid),
                # consistent with standard payroll practice.
                if is_sun:
                    wo_days += 1
                    continue
                # Half day leave: 0.5 leave + 0.5 present
                if is_hd:
                    paid_leave_days += 0.5
                    present_days    += 0.5
                    half_days       += 1
                ...
                # If this is the employee's weekly-off day but they punched in,
                # treat as WOP (Weekly Off Present = OT only, not a paid salary day)
                if is_sun:
                    wop_days += 1
                    ot_minutes += (rec.get("ot_minutes", 0) if rec else 0) or 0
                    continue
                ...
                if is_hd:
                    present_days += 0.5
                    half_days    += 1
                    # Credit 0.5 paid leave if an approved half-day leave exists for this date
                    try:
                        _hd_leave = conn.execute("""
                            SELECT id FROM leave_requests
                            WHERE emp_code=? AND is_half_day=1 AND status='Approved'
                              AND from_date <= ? AND to_date >= ?
                        """, (emp_code, dt_str, dt_str)).fetchone()
                        if _hd_leave:
                            paid_leave_days += 0.5
                    except: pass
                else:
                ...
                # Late mark — only if:
                # 1. late_minutes > 0 in attendance record
                # 2. NOT waived
                # 3. NOT a half-day (is_hd=1 means already penalised as half-day — no double penalty)
                # 4. No approved half-day leave for this date
                _rec_late   = ((rec.get("late_minutes", 0) if rec else 0) or 0) > 0
                _rec_waived = (rec.get("late_waived", 0) if rec else 0)
                if _rec_late and not _rec_waived and not is_hd:
                    _hd_leave_exists = False
                    try:
                        _hd_lr = conn.execute("""
                            SELECT id FROM leave_requests
                            WHERE emp_code=? AND is_half_day=1 AND status='Approved'
                              AND from_date <= ? AND to_date >= ?
                        """, (emp_code, dt_str, dt_str)).fetchone()
                        if _hd_lr: _hd_leave_exists = True
                    except: pass
                    if not _hd_leave_exists:
                        late_marks += 1
        ...
        esi_emp = esic_round_up(gross * (ps.get("esic_employee_pct", 0.75) / 100)) if (esi_app and gross <= esic_limit) else 0
        esi_er  = esic_round_up(gross * (ps.get("esic_employer_pct", 3.25) / 100)) if (esi_app and gross <= esic_limit) else 0
...
PERMISSION_TREE = [
    ...
    ("weekly_off",          "Attendance — Weekly Off Management", None,  1),
    ("weekly_off_bulk_upload","Attendance — Weekly Off Bulk Upload", None, 1),
    ...
    ("extra_working",       "Extra Working Hours",              None,  1),
    # Leave
    ...
    ("assoc_leave_apply",   "Associates — Leave (OT Auth) Apply", None, 1),
    ("assoc_leave_approve", "Associates — Approval Center (OT Auth)", None, 1),
    ...
    ("letter_approval",     "Letters — Approve Documents",      None,  1),
    ...
    ("my_career",           "My Career Journey (Self)",         None,  1),
    # HR/Management career module
    ("career_overview",     "Payroll — Career & Compensation Overview", None, 1),
]
...
def amgr(f):
    ...
    def w(*a,**k):
        ...
        self_service = ["/my/change-password", "/my/career", "/my-payslip", "/my-leaves", "/my-attendance",
        ...
        perm_map = [
            ...
            ("/weekly-off/bulk-upload",      ["weekly_off_bulk_upload"]),
            ("/weekly-off/download-template",["weekly_off_bulk_upload"]),
            ("/weekly-off",             ["weekly_off"]),
            ("/punch-alerts",           ["punch_alerts"]),
            ("/holidays/add",            ["holidays_edit"]),
            ("/holidays/edit",           ["holidays_edit"]),
            ("/holidays/delete",         ["holidays_edit"]),
            ...
            ("/extra-working/settings/save", ["extra_working"]),
            ("/extra-working/settings", ["extra_working"]),
            ("/extra-working/employee-detail", ["extra_working"]),
            ("/extra-working",          ["extra_working"]),
            ...
            ("/api/wages-data",                 ["leave_assoc", "payslip", "payroll_view"]),
            ("/assoc-leave/apply",    ["assoc_leave_apply", "dept_head_approve"]),
            ("/assoc-leave/approve",  ["assoc_leave_approve", "leave_assoc"]),
            ("/assoc-leave",          ["assoc_leave_apply", "assoc_leave_approve", "leave_assoc", "dept_head_approve"]),
            ...
            ("/salary/career-overview", ["career_overview", "salary_revision"]),
            ("/salary",                 ["payroll_view"]),
            ...
            ("/letters/settings/remove",     ["letter_admin"]),
            ("/letters/settings",            ["letter_admin"]),
            ("/letters/signatures",          ["letter_admin"]),
            ("/letters/field-types",          ["letters_view"]),
            ("/letters/templates/generate",  ["letters_view"]),
            ("/letters/templates/render",    ["letters_view"]),
            ("/letters/templates/preview",   ["letters_view"]),
            ("/letters/templates",           ["letter_admin"]),
            ("/letters/documents/approve",  ["letter_approval"]),
            ("/letters/documents/reject",   ["letter_approval"]),
            ("/letters/documents/edit",     ["letters_view"]),
            ("/letters/documents",          ["letters_view", "letter_approval"]),
        ...
        _edit_keywords = ("_add_edit", "_add", "_process", "_mark_paid", "_approve",
                          ...
                          "_leaves", "_attendance", "_apply", "_submit",
                          "payslip", "machines", "masters", "manpower",
                          "users", "dashboard", "exit_mgmt", "dept_head_approve")
...
def api_emp_info():
    ...
    conn = get_db()
    emp = conn.execute("SELECT emp_code,emp_name,department,category,basic,hra,special_allowance,designation FROM employees WHERE emp_code=?", (emp_code,)).fetchone()
    conn.close()
...
def api_designations():
    """Return all active designations from master for dropdown"""
    conn = get_db()
    desigs = [r["name"] for r in conn.execute("SELECT name FROM master_designations WHERE is_active=1 ORDER BY name").fetchall()]
    conn.close()
    return jsonify({"success": True, "designations": desigs})
...
def _employee_email_for(emp_id, conn):
    """Best available email for an employee: official > general > personal."""
    if not emp_id:
        return None
    row = conn.execute("SELECT email, personal_email, official_email FROM employees WHERE emp_code=?", (emp_id,)).fetchone()
    if not row:
        return None
    row = dict(row)
...
def _complete_login(user, perms):
    ...
    if user["role"] == "employee":
        if "dashboard" in _perms or "dashboard_kpis" in _perms:
            return redirect("/dashboard")
        return redirect("/my-leaves")
    if not _perms & {"dashboard","dashboard_kpis","payroll_view","att_register","emp_view"}:
        return redirect("/my-leaves")
    return redirect("/dashboard")

def _resolve_user_perms(user, conn):
    try:
        perm_rows = conn.execute("SELECT permission FROM user_permissions WHERE user_id=?", (user["id"],)).fetchall()
    ...
    if not perms:
        perms = ["my_payslip","my_leaves","my_attendance","my_career"]
    return perms

def _render_login(error=""):
    conn_b = get_db()
    brand = conn_b.execute("SELECT company_name, login_bg_type, login_headline, login_subtext FROM company_settings WHERE id=1").fetchone()
    conn_b.close()
    ...
    if request.method=="POST":
        ...
        if user and user["password"]==hp(p):
            if user["otp_login_enabled"]:
                ...
                conn_o = get_db()
                conn_o.execute("DELETE FROM login_otp WHERE user_id=?", (user["id"],))
                ...
                body = make_email_header() + f"""
                    <div style="padding:28px;font-family:Arial,sans-serif;">
                        ...
                        <p style="font-size:14px;color:#334155;">Use this One-Time Password to complete your login:</p>
                        ...
                        <p style="font-size:12px;color:#64748b;">This code expires in 5 minutes. If you didn't attempt to log in, you can ignore this email.</p>
                    </div>""" + make_email_footer()
                ok, msg = send_email(email, subject, body, email_type="login_otp")
                if not ok:
                    return _render_login(error=f"Could not send OTP email ({msg}). Contact admin.")
                masked = email[:2] + "***" + email[email.find("@"):] if "@" in email else email
                session["otp_pending_uid"] = user["id"]
                return render_template("login_otp.html", masked_email=masked, error="")
            conn2 = get_db()
            perms = _resolve_user_perms(user, conn2)
            conn2.close()
            return _complete_login(user, perms)
        err="Invalid username or password!"
    return _render_login(error=err)

@app.route("/login/verify-otp", methods=["POST"])
def login_verify_otp():
    uid = session.get("otp_pending_uid")
    if not uid:
    ...
    if not row:
        conn.close()
        return render_template("login_otp.html", masked_email=masked, error="OTP expired or not found. Please request a new one.")
    if row["attempts"] >= 5:
        conn.close()
        return render_template("login_otp.html", masked_email=masked, error="Too many attempts. Please request a new OTP.")
    if datetime.now() > datetime.strptime(row["expires_at"], "%Y-%m-%d %H:%M:%S"):
        conn.close()
        return render_template("login_otp.html", masked_email=masked, error="OTP expired. Please request a new one.")
    if code != row["otp_code"]:
        conn.execute("UPDATE login_otp SET attempts=attempts+1 WHERE id=?", (row["id"],))
        conn.commit(); conn.close()
        return render_template("login_otp.html", masked_email=masked, error="Incorrect OTP. Please try again.")
    # Success
    conn.execute("DELETE FROM login_otp WHERE user_id=?", (uid,))
    conn.commit()
    perms = _resolve_user_perms(user, conn)
    conn.close()
    session.pop("otp_pending_uid", None)
    return _complete_login(user, perms)
...
def payroll_trend_api():
    ...
    # Extra Working metric — only locked (finalized) months feed Payroll Insights
    if metric in ("extra_working_hours", "extra_working_amount"):
        ew_extra = ""
        ew_params = []
        if dept: ew_extra += " AND s.department=?"; ew_params.append(dept)
        if cat:  ew_extra += " AND e.category=?";   ew_params.append(cat)
        ew_rows = conn.execute(f"""
            SELECT s.month as mn, s.year as yr,
                   SUM(s.extra_hours) as total_hours,
                   SUM(s.extra_amount) as total_amount,
                   COUNT(DISTINCT s.emp_code) as emp_count
            FROM extra_working_snapshot s
            JOIN employees e ON s.emp_code=e.emp_code
            JOIN extra_working_lock l ON l.month=s.month AND l.year=s.year AND l.is_locked=1
            WHERE 1=1{ew_extra}
            GROUP BY s.year, s.month
            ORDER BY s.year DESC, s.month DESC
            LIMIT ?
        """, ew_params+[months_back]).fetchall()
        depts = [d["department"] for d in conn.execute(
            "SELECT DISTINCT department FROM employees WHERE status='Active' AND department IS NOT NULL ORDER BY department"
        ).fetchall()]
        conn.close()
        ew_rows = list(reversed(ew_rows))
        labels = [f"{mnames[(int(r['mn']) or 1)-1]} {r['yr']}" for r in ew_rows]
        if metric == "extra_working_hours":
            values = [round(float(r["total_hours"] or 0), 1) for r in ew_rows]
        else:
            values = [round(float(r["total_amount"] or 0), 2) for r in ew_rows]
        emp_counts = [r["emp_count"] for r in ew_rows]
...
def edit_emp(emp_code):
    ...
    try:
        ...
        # Weekly Off is managed solely via the Weekly Off Management module now;
        # preserve whatever is currently set instead of resetting to Sunday
        # just because this form no longer submits a weekly_off field.
        _cur_wo = conn.execute("SELECT weekly_off FROM employees WHERE emp_code=?", (emp_code,)).fetchone()
        _preserved_wo = (_cur_wo["weekly_off"] if _cur_wo and _cur_wo["weekly_off"] else "Sunday")
        conn.execute("""UPDATE employees SET emp_name=?,category=?,department=?,location=?,designation=?,
            ...
            (d["emp_name"],d["category"],new_dept,d.get("location",""),d.get("designation",""),
             ...
             float(d.get("tds_percent",0) or 0), scheme_id,
             d.get("uan_number",""),d.get("pf_number",""),d.get("esic_number",""),
             d.get("weekly_off", _preserved_wo),
...
def upload_emp_excel():
    ...
    try:
        ...
        for row in ws.iter_rows(min_row=data_start,values_only=True):
            ...
            _matched_wo = next((day for day in valid_days if day.lower() == wo_raw.lower()), None)
            if _matched_wo is not None:
                weekly_off = _matched_wo
            else:
                # No valid Weekly Off column in the sheet — don't overwrite;
                # Weekly Off is managed solely via the Weekly Off Management module.
                _existing_wo = conn.execute("SELECT weekly_off FROM employees WHERE emp_code=?", (ecode,)).fetchone()
                weekly_off = (_existing_wo["weekly_off"] if _existing_wo and _existing_wo["weekly_off"] else "Sunday")
    ...
    try:
        ...
        for pair in pairs:
            ...
            try:
                ...
                if att_dt_obj.weekday() == get_emp_weekly_off_num(emp_code, conn, on_date=att_dt_obj):
                    auto_status = "WOP"
    ...
    try:
        ...
        while cur <= month_end:
            ...
            if cur_str not in paired_dates:
                ...
                if cur.weekday() == get_emp_weekly_off_num(emp_code, conn, on_date=cur):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            try:
                ...
                if _dm.weekday() == get_emp_weekly_off_num(emp_code, conn, on_date=_dm):
                    status = "WOP"
            except:
                pass
            # Save attendance
            ...
            try:
                ...
                for pair in pairs:
                    ...
                    try:
                        ...
                        if att_dt.weekday() == get_emp_weekly_off_num(ec, conn, on_date=att_dt):
                            auto_status = "WOP"
                        else:
        ...
        for empcode, punch_list in emp_punches_sql.items():
            ...
            for pair in pairs:
                ...
                try:
                    from datetime import date as _dtc
                    att_dt = _dtc.fromisoformat(att_date)
                    auto_status = "WOP" if att_dt.weekday()==get_emp_weekly_off_num(ec, conn, on_date=att_dt) else "Present"
...
@amgr
def payroll_schemes():
    # Module removed from navigation — use Masters > Employee Schemes instead.
...
def payroll_ot_rates():
    # Module removed from navigation.
    return redirect("/dashboard")
    conn = get_db()
...
# ════════════════════════════════════════════════════════════
# EXTRA WORKING HOURS — Standalone module
# (separate from OT Rate Master / OT Process — untouched)
#
# Extra working hours = attendance.ot_minutes, already computed by
# calc_att() from each employee's Shift Master vs their IN/OUT punches
# (worked minutes beyond the shift's working_hours).
#
# Per-hour rate = (salary_basis_amount / days_divisor / shift_working_hours) * multiplier
# Amount = Extra Working Hours × Per-Hour Rate
# ════════════════════════════════════════════════════════════

def _get_extra_working_settings(conn):
    row = conn.execute("SELECT * FROM extra_working_settings WHERE id=1").fetchone()
    if not row:
        conn.execute("INSERT OR IGNORE INTO extra_working_settings (id, salary_basis, days_divisor, multiplier) VALUES (1,'gross',26,1.0)")
        conn.commit()
        row = conn.execute("SELECT * FROM extra_working_settings WHERE id=1").fetchone()
    return dict(row)
...
def _extra_working_per_hour_rate(emp, shift_hours, settings):
    basis = settings.get("salary_basis", "gross")
    if basis == "basic":
...
def _compute_extra_working_live(conn, m, y, dept="", scheme_id="", search=""):
    ...
    if scheme_id:
        sql += " AND e.scheme_id=?"; params.append(scheme_id)
    if search:
...
def _load_extra_working_snapshot(conn, m, y, dept="", scheme_id="", search=""):
    """Frozen data for a locked month — read-only, does not change even if attendance changes later."""
    sql = """SELECT s.emp_code, s.emp_name, s.department, e.category, e.scheme_id,
        s.extra_hours, s.per_hr_rate, s.extra_amount
        FROM extra_working_snapshot s
        JOIN employees e ON s.emp_code=e.emp_code
        WHERE s.month=? AND s.year=?"""
    params = [m, y]
    if dept:
        sql += " AND s.department=?"; params.append(dept)
    if scheme_id:
        sql += " AND e.scheme_id=?"; params.append(scheme_id)
    if search:
        sql += " AND (s.emp_code LIKE ? OR s.emp_name LIKE ?)"
        params += [f"%{search}%", f"%{search}%"]
    sql += " ORDER BY s.department, s.emp_name"
    rows = conn.execute(sql, params).fetchall()
    data = []
    for r in rows:
        r = dict(r)
...
@app.route("/extra-working")
@amgr
def extra_working_page():
    m      = int(request.args.get("month", date.today().month))
    y      = int(request.args.get("year",  date.today().year))
    dept   = request.args.get("dept", "")
    ...
    conn = get_db()
    settings = _get_extra_working_settings(conn)
    lock = _get_extra_working_lock(conn, m, y)

    if lock["is_locked"]:
        data = _load_extra_working_snapshot(conn, m, y, dept, scheme, search)
    else:
    ...
    schemes = conn.execute("SELECT id, scheme_name FROM employee_schemes WHERE is_active=1 ORDER BY scheme_name").fetchall()
...
@app.route("/extra-working/settings/save", methods=["POST"])
@amgr
def extra_working_settings_save():
    d = request.json or {}
    conn = get_db()
    try:
        salary_basis = d.get("salary_basis", "gross")
        if salary_basis not in ("gross", "basic"):
            salary_basis = "gross"
        days_divisor = float(d.get("days_divisor", 26))
        multiplier   = float(d.get("multiplier", 1.0))
        conn.execute("""INSERT INTO extra_working_settings (id, salary_basis, days_divisor, multiplier, updated_on)
            VALUES (1,?,?,?,date('now'))
            ON CONFLICT(id) DO UPDATE SET
                salary_basis=excluded.salary_basis,
                days_divisor=excluded.days_divisor,
                multiplier=excluded.multiplier,
                updated_on=excluded.updated_on""",
            (salary_basis, days_divisor, multiplier))
        conn.commit()
        return jsonify({"success": True})
    except Exception as e:
        return jsonify({"success": False, "error": str(e)})
...
@amgr
def extra_working_sync():
    """Recompute the live figures for a month right now (no external source — refresh trigger)."""
    d = request.json or {}
    ...
    try:
        ...
        if lock["is_locked"]:
            conn.close()
            return jsonify({"success": False, "error": f"{MONTHS[m-1]} {y} is locked. Unlock first to sync fresh data."})
        data = _compute_extra_working_live(conn, m, y)
        conn.close()
        total_hrs = round(sum(r["extra_hours"] for r in data), 2)
        return jsonify({"success": True,
            "message": f"Synced — {len(data)} employees, {total_hrs} total extra hours for {MONTHS[m-1]} {y}"})
    except Exception as e:
        conn.close()
        return jsonify({"success": False, "error": str(e)})

def _snapshot_extra_working(conn, m, y, by):
    """Compute live data and write it into extra_working_snapshot. Returns row count."""
    data = _compute_extra_working_live(conn, m, y)
    now = datetime.now().strftime("%Y-%m-%d %H:%M")
    for r in data:
        conn.execute("""INSERT INTO extra_working_snapshot
            (emp_code, emp_name, department, month, year, extra_hours, per_hr_rate, extra_amount, saved_on, saved_by)
            VALUES (?,?,?,?,?,?,?,?,?,?)
            ON CONFLICT(emp_code, month, year) DO UPDATE SET
                emp_name=excluded.emp_name, department=excluded.department,
                extra_hours=excluded.extra_hours, per_hr_rate=excluded.per_hr_rate,
                extra_amount=excluded.extra_amount, saved_on=excluded.saved_on, saved_by=excluded.saved_by""",
            (r["emp_code"], r["emp_name"], r["department"], m, y,
             r["extra_hours"], r["per_hr_rate"], r["extra_amount"], now, by))
    conn.commit()
    return len(data)

@app.route("/extra-working/save", methods=["POST"])
@amgr
def extra_working_save():
    """Save current calculated data as a draft snapshot (does not lock the month)."""
    d = request.json or {}
    m = int(d.get("month", date.today().month))
    y = int(d.get("year",  date.today().year))
    conn = get_db()
    try:
        ...
        if lock["is_locked"]:
            conn.close()
            return jsonify({"success": False, "error": f"{MONTHS[m-1]} {y} is already locked."})
...
@app.route("/extra-working/lock", methods=["POST"])
@amgr
def extra_working_lock_toggle():
    """Lock freezes the month (snapshot becomes source of truth, no further changes).
    Unlock allows recomputation again. Locking a month feeds its total into Payroll Insights."""
    d = request.json or {}
    m      = int(d.get("month", date.today().month))
    y      = int(d.get("year",  date.today().year))
    action = d.get("action", "lock")  # 'lock' or 'unlock'
    conn = get_db()
    try:
        by  = session.get("name", "HR")
        now = datetime.now().strftime("%Y-%m-%d %H:%M")
        if action == "unlock":
            conn.execute("""INSERT INTO extra_working_lock (month, year, is_locked, locked_by, locked_on)
                VALUES (?,?,0,?,?)
                ON CONFLICT(month, year) DO UPDATE SET is_locked=0, locked_by=excluded.locked_by, locked_on=excluded.locked_on""",
                (m, y, by, now))
            conn.commit()
            conn.close()
            ...
            conn.execute("""INSERT INTO extra_working_lock (month, year, is_locked, locked_by, locked_on)
                VALUES (?,?,1,?,?)
                ON CONFLICT(month, year) DO UPDATE SET is_locked=1, locked_by=excluded.locked_by, locked_on=excluded.locked_on""",
                (m, y, by, now))
            conn.commit()
            conn.close()
...
@app.route("/extra-working/export/<int:m>/<int:y>")
@amgr
def extra_working_export(m, y):
    """Excel export of the current month's Extra Working data (live or locked snapshot)."""
    import openpyxl
    ...
    search = request.args.get("search", "").strip()
    lock = _get_extra_working_lock(conn, m, y)
    data = _load_extra_working_snapshot(conn, m, y, dept, scheme, search) if lock["is_locked"] \
        else _compute_extra_working_live(conn, m, y, dept, scheme, search)
    conn.close()
    ...
    headers = ["Emp Code", "Name", "Department", "Extra Hours", "Per Hr Rate", "Extra Amount"]
    hdr_fill = PatternFill(start_color="0096DC", end_color="0096DC", fill_type="solid")
    ...
    row_i = 4
    for r in data:
        ws.cell(row=row_i, column=1, value=r["emp_code"])
        ws.cell(row=row_i, column=2, value=r["emp_name"])
        ws.cell(row=row_i, column=3, value=r.get("department") or "")
        ws.cell(row=row_i, column=4, value=r["extra_hours"])
        ws.cell(row=row_i, column=5, value=r["per_hr_rate"])
        ws.cell(row=row_i, column=6, value=r["extra_amount"])
        row_i += 1

    widths = [12, 24, 18, 14, 14, 16]
...
# ── Monthly Working Days — REMOVED ─────────────
# Working days are now always computed automatically from each employee's
# weekly_off setting: Total month days − count of weekly-off day occurrences.
# The manual override module has been removed.
...
def payroll_wd_delete(rid):
    return jsonify({"success": False, "error": "Working Days module has been removed. Working days are now auto-calculated from each employee's weekly off setting."})
...
def ot_process_page():
    ...
    for r in ot_rows:
        ...
        if gross_base <= 21000 and ot_amount > 0:
            esic_on_ot = esic_round_up(ot_amount * 0.0075)
...
def ot_bank_file(m, y):
    ...
    for r in ot_rows:
        ...
        esic   = esic_round_up(ot_amt * 0.0075) if gb <= 21000 else 0
...
def ot_esic_preview(m, y):
    ...
    for r in ot_rows:
        ...
        ot_hrs  = round((r["total_ot_min"] or 0) / 60, 2)
        ot_amt  = round((gb / 208) * 1.3 * ot_hrs, 2)
        emp_esi = esic_round_up(ot_amt * 0.0075)
        er_esi  = esic_round_up(ot_amt * 0.0325)
...
def leave_associate_page():
    ...
    for emp in all_nonstaff:
        ...
        for day in range(1, total_days + 1):
            d_obj = _dt(y, m, day)
            if d_obj.weekday() == get_emp_weekly_off_num(ec, conn, on_date=d_obj): continue
    ...
    for ec, emp in emp_map.items():
        ...
        # ESIC on OT
        esic_on_ot   = esic_round_up(net_ot_pay * 0.0075) if gross_base <= 21000 else 0
    ...
    # JSON response for Wages section in payslip
    if request.args.get("_json") == "1":
        return jsonify({"success":True, "emp_list":emp_list})
...
def leave_associate_save():
    ...
    try:
        ...
        for r in records:
            ...
            esic = esic_round_up(net_pay*0.0075) if gb<=21000 else 0
...
def leave_associate_export(m, y):
    ...
    for ec,e in sorted(emp_map.items()):
        ...
        esic_ot=esic_round_up(ot_pay*0.0075) if gb<=21000 else 0
...
def payroll_report_pf(m,y):
    """PF Report Export — matches statutory PF sheet format"""
    import openpyxl
    from openpyxl.styles import Font, PatternFill, Alignment, Border, Side
    from openpyxl.utils import get_column_letter
    conn = get_db()
    records = conn.execute("""SELECT sr.emp_code, e.emp_name, e.department, e.category,
        e.pf_number, e.uan_number, e.status as emp_status,
        sr.absent_days, sr.gross, sr.basic_earned, sr.pf, sr.employer_pf
        FROM salary_records sr JOIN employees e ON sr.emp_code=e.emp_code
    ...
    hdrs = ["S.No.","EPF NO.","UAN NO.","EMP Code","EMP NAME","STATUS","ABSENT DAYS",
            "GROSS SALARY","SALARY / WAGES","PENSION SALARY (LIMIT 15000/-)",
            "12% PF Deduction","8.33% Pension (Limit 15000*1250)","3.67% EPF Difference"]
    ...
    total_gross=total_wages=total_pf=total_pension=total_diff=0
    for i, r in enumerate(records, 1):
        gross = float(r["gross"] or 0)
        wages = float(r["basic_earned"] or 0)
        pension_salary = min(wages, 15000)
        pf_ded = float(r["pf"] or 0)
        pension_contrib = round(pension_salary * 0.0833, 0) if wages > 0 else 0
        epf_diff = max(0, pf_ded - pension_contrib)
        ws.append([
            i, r["pf_number"] or "—", r["uan_number"] or "—", r["emp_code"], r["emp_name"],
            r["emp_status"] or "Active", r["absent_days"] or 0,
            gross, wages, 15000 if wages>0 else 0,
            pf_ded, pension_contrib, epf_diff
        ])
        for cell in ws[ws.max_row]:
            cell.border = border
        total_gross+=gross; total_wages+=wages
        total_pf+=pf_ded; total_pension+=pension_contrib; total_diff+=epf_diff

    ws.append(["","","","","","","TOTAL",total_gross,total_wages,"",total_pf,total_pension,total_diff])
    for cell in ws[ws.max_row]:
        cell.font = Font(bold=True); cell.border = border

    widths = [6,14,14,10,22,10,12,14,14,16,14,18,16]
    for i,w in enumerate(widths,1):
        ws.column_dimensions[get_column_letter(i)].width = w
...
def payroll_report_esic(m,y):
    """ESIC Report Export — Salary ESIC + Overtime ESIC combined, matches statutory ESIC sheet format"""
    import openpyxl
    from openpyxl.styles import Font, PatternFill, Alignment, Border, Side
    from openpyxl.utils import get_column_letter
    conn = get_db()

    # Salary-side ESIC (present days, gross, esi)
    sal_records = conn.execute("""SELECT sr.emp_code, e.emp_name, e.esic_number,
        sr.present_days, sr.gross, sr.esi
        FROM salary_records sr JOIN employees e ON sr.emp_code=e.emp_code
        WHERE sr.month=? AND sr.year=?
        ORDER BY e.emp_name""", (m,y)).fetchall()
    sal_map = {r["emp_code"]: r for r in sal_records}

    # OT-side ESIC (from associate_ot_snapshot — saved OT processing data)
    ot_map = {}
    try:
        ot_records = conn.execute("""SELECT emp_code, net_ot_pay, esic
            FROM associate_ot_snapshot WHERE month=? AND year=?""", (m,y)).fetchall()
        ot_map = {r["emp_code"]: r for r in ot_records}
    except Exception:
        ot_map = {}

    conn.close()

    # Union of all employee codes that have either salary ESIC or OT ESIC
    all_codes = []
    seen = set()
    for r in sal_records:
        if r["emp_code"] not in seen:
            all_codes.append(r["emp_code"]); seen.add(r["emp_code"])
    for code in ot_map:
        if code not in seen:
            all_codes.append(code); seen.add(code)
    ...
    # Section header row — groups Salary cols and Over Time cols
    ws.append([])  # row 3 spacer
    ...
    hdrs = ["S.No.","IP NO.","EMP Code","NAME OF EMPLOYEE",
            "PRESENT DAYS","GROSS SALARY","ESIC DEDUCTION",
            "Earn Amount OT","ESIC DEDUCTION","TOTAL ESIC (Salary+OT)"]
    ws.append(hdrs)
    for cell in ws[5]:
        cell.font = hdr_font; cell.fill = hdr_fill; cell.border = border
        cell.alignment = Alignment(horizontal="center", vertical="center", wrap_text=True)
    ...
    for code in all_codes:
        ...
        ot  = ot_map.get(code)
        if not sal and not ot:
            continue
        row_no += 1
        emp_name = sal["emp_name"] if sal else "—"
        ip_no    = (sal["esic_number"] if sal and sal["esic_number"] else "—")
        present_days = float(sal["present_days"] or 0) if sal else 0
        gross_sal    = float(sal["gross"] or 0) if sal else 0
        sal_esic     = float(sal["esi"] or 0) if sal else 0
        ot_amt       = float(ot["net_ot_pay"] or 0) if ot else 0
        ot_esic      = float(ot["esic"] or 0) if ot else 0
        row_total    = sal_esic + ot_esic

        ws.append([row_no, ip_no, code, emp_name, present_days,
                   gross_sal, sal_esic, ot_amt, ot_esic, row_total])
        for cell in ws[ws.max_row]:
            cell.border = border

        total_sal_esic += sal_esic
        total_ot_esic  += ot_esic
        grand_total    += row_total

    ws.append(["","","","TOTAL","","",total_sal_esic,"",total_ot_esic,grand_total])
    for cell in ws[ws.max_row]:
        cell.font = Font(bold=True); cell.border = border

    widths = [6,14,10,24,12,14,14,14,14,18]
    for i,w in enumerate(widths,1):
        ws.column_dimensions[get_column_letter(i)].width = w
...
def do_increment():
    """
    Salary Increment / Designation Change:
    - salary_change=True  → applies increment on ACTUAL GROSS (Basic+HRA+Special)
    - salary_change=False → designation-only change, salary untouched
    - new_designation set → updates designation on employee master + logs in increments
    ...
    try:
        ...
        do_salary   = bool(d.get("salary_change", True))  # default True for backward compat
        inc_amount  = 0.0
        new_basic   = old_basic
        new_hra     = old_hra
        ...
        if do_salary:
            ...
            if inc_value <= 0:
                return jsonify({"success":False,"error":"Increment value must be greater than 0"})
            ...
            # ── Admin override: explicit Basic + HRA values ──────────────
            override_basic = d.get("override_basic")
            override_hra   = d.get("override_hra")
            if override_basic is not None and override_hra is not None:
                ob = round(float(override_basic), 2)
                oh = round(float(override_hra), 2)
                # Validate: basic + hra must not exceed new_gross
                if ob + oh > new_gross + 0.02:
                    conn.close()
                    return jsonify({"success": False,
                        "error": f"Override Basic (₹{ob:,.0f}) + HRA (₹{oh:,.0f}) = ₹{ob+oh:,.0f} exceeds New Gross ₹{new_gross:,.0f}. Please reduce."})
                new_basic = ob
                new_hra   = oh
        ...
        old_designation = emp["designation"] or ""
        new_designation = str(d.get("new_designation","") or "").strip()
        if new_designation and new_designation != old_designation:
            conn.execute("UPDATE employees SET designation=? WHERE emp_code=?",
                         (new_designation, d["emp_code"]))
        else:
            new_designation = ""  # No designation change

        # Must have at least one change
        if not do_salary and not new_designation:
            return jsonify({"success":False,"error":"Please select Salary Increment and/or Designation Change"})

        conn.execute("""INSERT INTO increments
            (emp_code,effective_date,old_basic,new_basic,reason,done_on,old_designation,new_designation)
            VALUES (?,?,?,?,?,?,?,?)""",
            (d["emp_code"],
             ...
             old_designation if new_designation else "",
             new_designation))
        conn.commit()
        return jsonify({
            "success":True,
            "salary_changed": do_salary,
...
def payslip():
    ...
    y   = int(request.args.get("year", date.today().year))
    cat = request.args.get("cat","")
    slip_type = request.args.get("type","salary")  # salary | wages
    conn = get_db()
    ...
    return render_template("payslip.html", records=recs, month=m, year=y, months=MONTHS,
        month_name=MONTHS[m-1], cat_filter=cat, slip_type=slip_type,
        has_header=has_header, has_footer=has_footer, has_seal=has_seal)

@app.route("/payslip/wages")
@amgr
def payslip_wages():
    """Wages slip page for Associates — uses same OT data as Overtime Processing"""
    m      = int(request.args.get("month", date.today().month))
    y      = int(request.args.get("year",  date.today().year))
    dept   = request.args.get("dept", "")
    scheme = request.args.get("scheme", "")
    ...
    conn = get_db()
    try:
        # Reuse exact same logic as leave_associate_page
        dept_filter      = " AND e.department=?" if dept else ""
        dept_filter_bare = " AND department=?"   if dept else ""
        dept_params      = [dept] if dept else []
        scheme_filter = ""; scheme_filter_bare = ""; scheme_params = []
        if scheme:
            _sc = conn.execute("SELECT id FROM employee_schemes WHERE scheme_name=? AND is_active=1",(scheme,)).fetchone()
            if _sc:
                scheme_filter      = " AND e.scheme_id=?"
                scheme_filter_bare = " AND scheme_id=?"
                scheme_params      = [_sc["id"]]
        ...
        ot_rows = conn.execute(f"""
            ...
            WHERE strftime('%m',a.att_date)=? AND strftime('%Y',a.att_date)=?
              AND e.category='Associate' AND e.status='Active'
              AND a.ot_minutes>0{dept_filter}{scheme_filter}
            GROUP BY a.emp_code
        """, [f"{m:02d}", str(y)] + dept_params + scheme_params).fetchall()
        ot_by_emp = {r["emp_code"]: (r["total_ot_min"] or 0) for r in ot_rows}

        # Approved absent dates
        approved_rows = conn.execute(f"""
            SELECT alr.emp_code, COUNT(*) as cnt
            FROM associate_leave_records alr
            JOIN employees e ON alr.emp_code=e.emp_code
            WHERE strftime('%m',alr.absent_date)=? AND strftime('%Y',alr.absent_date)=?
              AND alr.leave_status='Approved' AND e.category='Associate'{dept_filter}{scheme_filter}
            GROUP BY alr.emp_code
        """, [f"{m:02d}", str(y)] + dept_params + scheme_params).fetchall()
        approved_by_emp = {r["emp_code"]: r["cnt"] for r in approved_rows}

        # All associates
        # Build month-end date to filter employees who were active during selected month
        import calendar as _cal2
        month_end = f"{y}-{m:02d}-{_cal2.monthrange(y, m)[1]:02d}"
        month_start = f"{y}-{m:02d}-01"

        all_assoc = conn.execute(f"""
            SELECT emp_code, emp_name, department, designation, category,
                   esic_number, phone, basic, hra, special_allowance, scheme_id
            FROM employees WHERE category='Associate'
              AND (status='Active' OR (last_working_day IS NOT NULL AND last_working_day >= ?))
              AND (date_of_joining IS NULL OR date_of_joining <= ?)
              {dept_filter_bare}{scheme_filter_bare}
            ORDER BY emp_name
        """, [month_start, month_end] + dept_params + scheme_params).fetchall()

        # Absent count
        absent_rows = conn.execute(f"""
            SELECT a.emp_code, COUNT(*) as cnt
            FROM attendance a JOIN employees e ON a.emp_code=e.emp_code
            WHERE strftime('%m',a.att_date)=? AND strftime('%Y',a.att_date)=?
              AND e.category='Associate' AND a.status='Absent'{dept_filter}{scheme_filter}
            GROUP BY a.emp_code
        """, [f"{m:02d}", str(y)] + dept_params + scheme_params).fetchall()
        absent_by_emp = {r["emp_code"]: r["cnt"] for r in absent_rows}

        depts   = conn.execute("SELECT DISTINCT department FROM employees WHERE status='Active' AND category='Associate' AND department IS NOT NULL ORDER BY department").fetchall()
        schemes = conn.execute("SELECT scheme_name FROM employee_schemes WHERE is_active=1 ORDER BY scheme_name").fetchall()

        emp_list = []
        for e in all_assoc:
            ec = e["emp_code"]
            gross_base = float(e["basic"] or 0) + float(e["hra"] or 0) + float(e["special_allowance"] or 0)
            actual_ot_hrs = round((ot_by_emp.get(ec, 0)) / 60, 2)
            total_absent  = absent_by_emp.get(ec, 0)
            approved_days = approved_by_emp.get(ec, 0)
            unauth_days   = max(0, total_absent - approved_days)

            unauth_ot_hrs   = min(actual_ot_hrs, 8 * unauth_days)
            approved_ot_hrs = max(0, round(actual_ot_hrs - unauth_ot_hrs, 2))

            ot_rate     = (gross_base / 208) * 1.3
            single_rate = gross_base / 208
            approved_ot_pay = round(ot_rate * approved_ot_hrs, 2)
            unauth_ot_pay   = round(single_rate * unauth_ot_hrs, 2)
            net_ot_pay      = round(approved_ot_pay + unauth_ot_pay, 2)
            esic_on_ot      = esic_round_up(net_ot_pay * 0.0075) if gross_base <= 21000 else 0
            net_ot_final    = round(net_ot_pay - esic_on_ot, 2)
            ...
            emp_list.append({
                ...
                "actual_ot_hrs": actual_ot_hrs,
                "approved_ot_hrs": approved_ot_hrs,
                "net_ot_pay": net_ot_pay,
                "esic_on_ot": esic_on_ot,
                "net_ot_final": net_ot_final,
            })

        return render_template("payslip_wages.html",
            emp_list=emp_list,
            month=m, year=y, month_name=MONTHS[m-1], months=MONTHS,
            departments=[d["department"] for d in depts],
            schemes=[s["scheme_name"] for s in schemes],
            selected_dept=dept, selected_scheme=scheme)
    finally:
        conn.close()
...
def get_payslip(emp_code,month,year):
    ...
    conn=get_db()
    # Employee self-service: only show payslip if month is locked
    if session.get("role")=="employee":
        is_locked = conn.execute(
            "SELECT id FROM payroll_locks WHERE month=? AND year=?", (month,year)
        ).fetchone()
        if not is_locked:
            conn.close()
            return jsonify({"error":"Payslip not yet available. Salary processing for this month is pending."})
    conn=get_db()
...
def my_payslip():
    ...
    emp_increments = conn.execute("SELECT * FROM increments WHERE emp_code=? ORDER BY effective_date DESC",(eid,)).fetchall()
    conn.close()
    return render_template("my_payslip.html",records=recs,emp=emp,months=MONTHS,emp_increments=emp_increments)


@app.route("/my/career")
@lreq
def my_career():
    """Employee self-service: My Career Journey — salary & designation history"""
    eid = session.get("emp_id","").strip() if session.get("emp_id") else ""
    conn = get_db()
    if not eid:
        conn.close()
        return render_template("my_career.html", emp=None, milestones=[], records=[], months=MONTHS,
            error="No employee linked to your account. Contact HR.")
    emp = conn.execute("SELECT * FROM employees WHERE emp_code=?",(eid,)).fetchone()
    if not emp:
        conn.close()
    ...
    milestones = conn.execute(
        "SELECT * FROM increments WHERE emp_code=? ORDER BY effective_date ASC",(eid,)).fetchall()
    records = conn.execute(
        """SELECT s.* FROM salary_records s
           JOIN payroll_locks pl ON pl.month=s.month AND pl.year=s.year
           WHERE s.emp_code=? ORDER BY s.year DESC,s.month DESC LIMIT 12""",(eid,)).fetchall()
    conn.close()
    # Monthly CTC (same formula used for offer/appointment letters) — Basic+HRA+Special + Employer PF + Employer ESI + Bonus + Gratuity/mo
    emp_d = dict(emp)
    basic = emp_d.get("basic", 0) or 0
    hra = emp_d.get("hra", 0) or 0
    special = emp_d.get("special_allowance", 0) or 0
    base_gross = basic + hra + special
    bonus_calc = round(basic * 0.0833, 2)
    employer_pf_calc = round(basic * 0.13, 2) if emp_d.get("pf_applicable", 1) else 0
    employer_esi_calc = ((esic_round_up(base_gross * 0.0325) if base_gross <= 21000 else 0)
                          if emp_d.get("esi_applicable", 1) else 0)
    gratuity_m = round((basic / 26 * 15) / 12, 2) if basic else 0
    monthly_ctc = round(base_gross + employer_pf_calc + employer_esi_calc + bonus_calc + gratuity_m, 2)
    return render_template("my_career.html", emp=emp_d, milestones=milestones,
                           records=records, months=MONTHS, monthly_ctc=monthly_ctc)


@app.route("/salary/career-overview")
@amgr
def career_overview():
    """HR/Management: Career & Compensation Overview — all employees"""
    conn = get_db()
    emps = conn.execute(
        "SELECT emp_code,emp_name,department,category,designation,basic,hra,special_allowance,date_of_joining,status FROM employees WHERE status='Active' ORDER BY emp_name"
    ).fetchall()
    increments = conn.execute(
        """SELECT i.*, e.emp_name, e.department, e.category, e.designation
           FROM increments i JOIN employees e ON i.emp_code=e.emp_code
           ORDER BY i.effective_date DESC"""
    ).fetchall()
    # Full employee list (all statuses) — used for Joining / Left(Resigned) KPIs
    all_emps = conn.execute(
        """SELECT emp_code,emp_name,department,category,gender,date_of_joining,
                  status,resignation_date,last_working_day
           FROM employees ORDER BY emp_name"""
    ).fetchall()
    # Locked salary records — used for Salary + OT Total Earn KPI
    earn_records = conn.execute(
        """SELECT s.emp_code, e.emp_name, e.department, e.category, s.month, s.year,
                  s.gross, s.ot_amount
           FROM salary_records s
           JOIN employees e ON s.emp_code=e.emp_code
           JOIN payroll_locks pl ON pl.month=s.month AND pl.year=s.year
    ...
    departments = sorted(set(e["department"] for e in emps if e["department"]))
    categories  = sorted(set(e["category"] for e in emps if e["category"]))
    # Distinct years present across joining/exit/salary dates — for the Month/Year filter
    years_set = set()
    for e in all_emps:
        for fld in ("date_of_joining", "resignation_date", "last_working_day"):
            v = e[fld]
            if v and len(str(v)) >= 4:
                try: years_set.add(int(str(v)[:4]))
                except: pass
    for r in earn_records:
        if r["year"]: years_set.add(r["year"])
    years_list = sorted(years_set, reverse=True)
    conn.close()
    return render_template("career_overview.html",
                           emps=emps, increments=increments,
                           all_emps=all_emps, earn_records=earn_records, years_list=years_list,
                           departments=departments, categories=categories, months=MONTHS)
...
def process_exit():
    ...
    try:
        ...
        # ── Leave Encashment at Full & Final: encash unused EL + CL, then zero the balance ──
        leave_encash_total = 0.0
        leave_encash_breakup = []
        fy_year = get_leave_fy_year(conn, lwd)
        bal = conn.execute("SELECT * FROM leave_balance WHERE emp_code=? AND year=?",
                            (d["emp_code"], fy_year)).fetchone()
        if bal:
            per_day = float((emp["basic"] or 0)) / 26
            for code, label in (("el","EL"),("cl","CL")):
                carried = float(bal["el_carried"] or 0) if code=="el" else 0.0
                unused = round(float(bal[f"{code}_allotted"] or 0) + carried - float(bal[f"{code}_used"] or 0), 2)
                if unused <= 0:
                    continue
                already = conn.execute(
                    "SELECT id FROM leave_year_end_log WHERE emp_code=? AND year=? AND leave_type_code=?",
                    (d["emp_code"], fy_year, label)).fetchone()
                if already:
                    continue  # already carried/encashed via Year-End process — don't double-encash
                amount = round(per_day * unused, 2)
                conn.execute(f"UPDATE leave_balance SET {code}_used={code}_used+? WHERE emp_code=? AND year=?",
                    (unused, d["emp_code"], fy_year))
                conn.execute("""INSERT INTO leave_requests
                    ...
                    VALUES (?,?,?,?,?,?,?,'Approved',?,?,datetime('now'),'encashment',?)""",
                    (d["emp_code"], emp["emp_name"], label, lwd.isoformat(), lwd.isoformat(), unused,
                     "Full & Final Settlement", datetime.now().strftime("%Y-%m-%d %H:%M"),
                     session.get("name","HR"), f"F&F Encashment: Rs.{amount:,.2f} for {unused} {label} days."))
                conn.execute("""INSERT INTO leave_year_end_log
                    (emp_code,year,leave_type_code,action,days,amount,processed_by,processed_on)
                    VALUES (?,?,?,'fnf_encash',?,?,?,datetime('now'))""",
                    (d["emp_code"], fy_year, label, unused, amount, session.get("name","HR")))
                leave_encash_total += amount
                leave_encash_breakup.append({"type":label,"days":unused,"amount":amount})
        leave_encash_total = round(leave_encash_total, 2)
        conn.execute("UPDATE employees SET leave_encashment_amount=? WHERE emp_code=?",
                     (leave_encash_total, d["emp_code"]))
        conn.commit()
        conn.close()
        return jsonify({"success":True,"years_of_service":years,"gratuity":gratuity,
            "leave_encashment_amount":leave_encash_total,"leave_encashment_breakup":leave_encash_breakup})
    except Exception as e: conn.rollback(); conn.close(); return jsonify({"success":False,"error":str(e)})
...
def full_final(emp_code):
    ...
    leave_encashment=float(emp["leave_encashment_amount"] or 0) if "leave_encashment_amount" in emp.keys() else 0
...
def export_ctc():
    ...
    for r in rows:
        ...
        ot_esic   = esic_round_up(ot_gross * 0.0075) if base_gross <= 21000 and ot_gross > 0 else 0
        epf_c     = round(base_basic * 0.13, 2)
        eesi_c    = esic_round_up(base_gross * 0.0325) if base_gross <= 21000 else 0
...
def user_permissions_route(uid):
    ...
    if request.method == "POST":
        ...
        otp_login = 1 if d.get("otp_login_enabled") else 0
        try:
            ...
            conn.execute("UPDATE users SET otp_login_enabled=? WHERE id=?", (otp_login, uid))
            conn.commit()
        except Exception as e:
            conn.close()
            return jsonify({"success":False,"error":str(e)})
    ...
    try:
        ...
        dept_access = [r["department"] for r in dept_rows]
        conn.close()
        return jsonify({"success": True, "permissions": list(perms), "dept_access": dept_access,
            "username": user["username"], "otp_login_enabled": bool(user["otp_login_enabled"])})
    except Exception as e:
        conn.close()
...
def admin_settings():
    ...
    if request.method == "POST":
        ...
        bg = request.files.get("login_bg")
        if bg and bg.filename:
            bg_data = bg.read()
            mimetype = bg.mimetype or ""
            ...
            if not (is_video or is_image):
                ...
                conn.execute("""UPDATE company_settings SET
                    ...
                    WHERE id=1""", (bg_data, bg.filename, mimetype, "video" if is_video else "image"))
                conn.commit()
                flash_msg = (flash_msg + " " if flash_msg else "") + "Login background updated!"

        headline = request.form.get("login_headline")
        subtext  = request.form.get("login_subtext")
        if headline is not None or subtext is not None:
            conn.execute("""UPDATE company_settings SET
                login_headline=COALESCE(?, login_headline),
                login_subtext=COALESCE(?, login_subtext),
                updated_on=datetime('now') WHERE id=1""",
                (headline if headline is not None else None, subtext if subtext is not None else None))
            conn.commit()
            flash_msg = (flash_msg + " " if flash_msg else "") + "Branding text updated!"

        if not flash_msg:
            flash_msg = "No changes submitted"
...
@app.route("/admin/login-bg")
def login_bg_media():
    conn = get_db()
    row = conn.execute("SELECT login_bg, login_bg_mimetype FROM company_settings WHERE id=1").fetchone()
    conn.close()
    if row and row["login_bg"]:
        from flask import Response
        return Response(row["login_bg"], mimetype=row["login_bg_mimetype"] or "image/jpeg",
            headers={"Cache-Control":"public,max-age=3600"})
    return ("", 204)

@app.route("/admin/login-bg/delete", methods=["POST"])
@amgr
def delete_login_bg():
    conn = get_db()
    conn.execute("""UPDATE company_settings SET
        login_bg=NULL, login_bg_filename=NULL, login_bg_mimetype=NULL, login_bg_type=NULL
        WHERE id=1""")
    conn.commit(); conn.close()
    return jsonify({"success": True})
...
def exp_salary(m,y):
    ...
    if scheme_f:
        ...
        if sc:
            # Include: (active employees in this scheme) OR (inactive employees with salary processed AND gross>=1 in this scheme)
            scheme_sql = " AND (e.scheme_id=? OR (e.status!='Active' AND COALESCE(s.gross,0)>=1 AND e.scheme_id=?))"
            exp_params.append(sc["id"]); exp_params.append(sc["id"])
    conn=get_db()
    rows=conn.execute(f"""SELECT s.emp_code,e.emp_name,e.category,e.department,e.designation,
        ...
        FROM salary_records s JOIN employees e ON s.emp_code=e.emp_code
        WHERE s.month=? AND s.year=?
        AND (e.status='Active' OR COALESCE(s.gross,0) >= 1)
        {cat_sql}{dept_sql}{scheme_sql} ORDER BY e.category,e.emp_name""",exp_params).fetchall()
    conn.close()
    data_rows = []
    for r in rows:
...
def leave_master():
    ...
    return render_template("leave_master.html",
        ...
        earn_auto_credit=int(settings["earn_auto_credit"]) if settings and settings["earn_auto_credit"] is not None else 0,
        leave_types=[dict(lt) for lt in leave_types],
        months=MONTHS)
...
def apply_leave():
    ...
    try:
        ...
        if session.get("role") == "employee":
            min_allowed = (date.today() - timedelta(days=30)).strftime("%Y-%m-%d")
            if from_date < min_allowed:
                return jsonify({"success": False,
                    "error": f"Leave can only be applied up to 30 days in the past. For older dates, please contact HR."})
...
def approve_leave(leave_id):
    ...
    try:
        ...
        # Use leave type already selected by employee — no need to re-assign
        assigned_type = leave["leave_type"]
        if not assigned_type or assigned_type == "Pending":
            assigned_type = d.get("leave_type","")
        if not assigned_type:
            return jsonify({"success": False, "error": "Leave type not set on this request. Please edit the request first."})
        ...
        if emp:
            ...
            while current <= td:
                ...
                if leave["is_half_day"]:
                    ...
                    if existing:
                        # FIX: status must also be set to 'Leave' (not left as 'Present')
                        # so payroll engine correctly counts 0.5 leave + 0.5 present,
                        # and does not penalize the day as a chargeable late/short-time mark.
                        conn.execute("UPDATE attendance SET is_half_day=1, status='Leave' WHERE emp_code=? AND att_date=?",
                                    (leave["emp_code"], att_date))
                    else:
                        conn.execute("""INSERT OR IGNORE INTO attendance
                            (emp_code,att_date,status,is_half_day) VALUES (?,?,'Leave',1)""",
                            (leave["emp_code"], att_date))
                else:
                    conn.execute("""INSERT OR REPLACE INTO attendance 
                        (emp_code,att_date,in_time,out_time,working_minutes,status,
...
@app.route("/leaves/repair-halfday-status", methods=["POST"])
@amgr
def repair_halfday_leave_status():
    """
    One-time data-repair utility.
    Fixes historical half-day leave approvals where attendance.status was
    left as 'Present' instead of 'Leave' (old bug — already fixed for new
    approvals). This caused payroll to wrongly treat those half-days as a
    full/penalizable Present day instead of 0.5 Leave + 0.5 Present,
    leading to incorrect late/short-time deductions.
    Safe to run multiple times — only touches rows that are still wrong.
    """
    conn = get_db()
    try:
        # Find all approved half-day leave requests
        leaves = conn.execute(
            "SELECT emp_code, from_date, to_date FROM leave_requests WHERE status='Approved' AND is_half_day=1"
        ).fetchall()
        fixed = 0
        for lv in leaves:
            fd = datetime.strptime(lv["from_date"], "%Y-%m-%d").date()
            td = datetime.strptime(lv["to_date"], "%Y-%m-%d").date()
            current = fd
            while current <= td:
                att_date = current.strftime("%Y-%m-%d")
                row = conn.execute(
                    "SELECT status FROM attendance WHERE emp_code=? AND att_date=? AND is_half_day=1",
                    (lv["emp_code"], att_date)
                ).fetchone()
                if row and row["status"] != "Leave":
                    conn.execute(
                        "UPDATE attendance SET status='Leave' WHERE emp_code=? AND att_date=? AND is_half_day=1",
                        (lv["emp_code"], att_date)
                    )
                    fixed += 1
                current += timedelta(days=1)
        conn.commit()
        return jsonify({"success": True, "fixed_records": fixed})
    except Exception as e:
        return jsonify({"success": False, "error": str(e)})
    finally:
        conn.close()
...
@app.route("/leaves/card/<emp_code>")
@amgr
def leave_card(emp_code):
    """Employee-wise Leave Card: Opening/Earned/Availed/Balance summary (CL+EL) plus a
    complete chronological transaction ledger with running balance — for the Leave Card popup."""
    year = int(request.args.get("year", date.today().year))
    conn = get_db()
    try:
        emp = conn.execute("SELECT * FROM employees WHERE emp_code=?", (emp_code,)).fetchone()
        if not emp:
            conn.close(); return jsonify({"success": False, "error": "Employee not found"})
        emp = dict(emp)
        bal = conn.execute("SELECT * FROM leave_balance WHERE emp_code=? AND year=?", (emp_code, year)).fetchone()
        bal = dict(bal) if bal else {}
        lms = conn.execute("SELECT financial_year_start_month FROM leave_master_settings WHERE id=1").fetchone()
        fy_start = int(lms["financial_year_start_month"]) if lms and lms["financial_year_start_month"] else 4
        period_from = date(year, fy_start, 1)
        period_to   = date(year + 1, fy_start, 1) - timedelta(days=1)
        period_label = f"{period_from.strftime('%b %Y')} – {period_to.strftime('%b %Y')}"

        summary = {}
        for code, label in (("cl", "CL"), ("el", "EL")):
            earned = conn.execute(f"SELECT COALESCE(SUM({code}_credited),0) FROM leave_earn_log WHERE emp_code=? AND year=?",
                                   (emp_code, year)).fetchone()[0]
            availed = conn.execute("""SELECT COALESCE(SUM(days),0) FROM leave_requests
                WHERE emp_code=? AND leave_type=? AND status='Approved' AND (source IS NULL OR source!='encashment')
                AND from_date>=? AND from_date<=?""",
                (emp_code, label, period_from.isoformat(), period_to.isoformat())).fetchone()[0]
            opening = float(bal.get("el_carried", 0) or 0) if code == "el" else 0.0
            balance = round(float(bal.get(f"{code}_allotted", 0) or 0) + opening - float(bal.get(f"{code}_used", 0) or 0), 2)
            summary[label] = {"opening": round(opening, 2), "earned": round(float(earned or 0), 2),
                               "availed": round(float(availed or 0), 2), "balance": balance}

        # Transaction ledger: Availed/Encashed (debit) + Earned (credit), chronological
        txns = []
        for r in conn.execute("""SELECT * FROM leave_requests WHERE emp_code=? AND status='Approved'
            AND leave_type IN ('CL','EL') AND from_date>=? AND from_date<=? ORDER BY from_date""",
            (emp_code, period_from.isoformat(), period_to.isoformat())).fetchall():
            r = dict(r)
            particular = "Leave Encashment" if r.get("source") == "encashment" else "Leave Availed"
            txns.append({"date": r["from_date"], "particular": particular, "leave_type": r["leave_type"],
                "credit": 0, "debit": float(r["days"] or 0), "remarks": r.get("remarks") or r.get("reason") or ""})
        for r in conn.execute("SELECT * FROM leave_earn_log WHERE emp_code=? AND year=? ORDER BY month",
                               (emp_code, year)).fetchall():
            r = dict(r)
            txn_date = f"{year if r['month'] >= fy_start else year + 1}-{int(r['month']):02d}-01"
            if r.get("el_credited"):
                txns.append({"date": txn_date, "particular": "Leave Earned", "leave_type": "EL",
                    "credit": float(r["el_credited"]), "debit": 0, "remarks": r.get("remarks") or "",
                    "log_id": r["id"], "month": r["month"]})
            if r.get("cl_credited"):
                txns.append({"date": txn_date, "particular": "Leave Earned", "leave_type": "CL",
                    "credit": float(r["cl_credited"]), "debit": 0, "remarks": r.get("remarks") or "",
                    "log_id": r["id"], "month": r["month"]})
        txns.sort(key=lambda t: t["date"])
        running = {"CL": summary["CL"]["opening"], "EL": summary["EL"]["opening"]}
        for t in txns:
            running[t["leave_type"]] = round(running[t["leave_type"]] + t["credit"] - t["debit"], 2)
            t["running_balance"] = running[t["leave_type"]]

        conn.close()
        return jsonify({"success": True,
            "employee": {"emp_code": emp["emp_code"], "emp_name": emp["emp_name"],
                "department": emp.get("department"), "designation": emp.get("designation"),
                "date_of_joining": emp.get("date_of_joining"), "category": emp.get("category")},
            "year": year, "period_label": period_label, "summary": summary, "transactions": txns,
            "is_admin": session.get("role") == "admin"})
    except Exception as e:
        conn.close()
        return jsonify({"success": False, "error": str(e)})

@app.route("/leaves/earn-log/edit", methods=["POST"])
def leave_earn_log_edit():
    """Rewrite an already-credited month's EL/CL Earn value. ADMIN ACCOUNT ONLY —
    e.g. June had 1.33 EL earned, Admin corrects it to 1 → leave_earn_log AND the
    employee's running leave_balance are both updated together (kept in sync).
    Still respects the yearly EL/CL cap (Payroll Settings)."""
    if "user" not in session:
        return jsonify({"success": False, "error": "Please login."}), 401
    if session.get("role") != "admin":
        return jsonify({"success": False, "error": "You are not authorized to perform this action. Admin account only."}), 403
    d = request.json or {}
    log_id = d.get("log_id")
    leave_type = str(d.get("leave_type", "")).upper()
    if not log_id or leave_type not in ("EL", "CL"):
        return jsonify({"success": False, "error": "Invalid request."})
    try:
        new_value = float(d.get("new_value"))
    except (TypeError, ValueError):
        return jsonify({"success": False, "error": "Please enter a valid value."})
    if new_value < 0:
        return jsonify({"success": False, "error": "Value cannot be negative."})

    conn = get_db()
    try:
        log = conn.execute("SELECT * FROM leave_earn_log WHERE id=?", (log_id,)).fetchone()
        if not log:
            conn.close(); return jsonify({"success": False, "error": "Leave earn record not found."})
        code = leave_type.lower()
        old_value = float(log[f"{code}_credited"] or 0)

        el_cap, cl_cap = get_leave_earn_caps(conn)
        cap = el_cap if code == "el" else cl_cap
        bal = conn.execute("SELECT * FROM leave_balance WHERE emp_code=? AND year=?",
                            (log["emp_code"], log["year"])).fetchone()
        current_allotted = float(bal[f"{code}_allotted"] or 0) if bal else 0.0
        # Clip so the rewrite never pushes the employee's yearly total past the cap
        room_excl_this_entry = max(0.0, cap - (current_allotted - old_value))
        final_value = min(new_value, room_excl_this_entry)
        delta = round(final_value - old_value, 4)

        conn.execute(f"UPDATE leave_earn_log SET {code}_credited=?, remarks=? WHERE id=?",
            (final_value, f"[Rewritten by {session.get('name','Admin')} on {datetime.now().strftime('%Y-%m-%d %H:%M')}: {old_value} → {final_value}]", log_id))
        conn.execute(f"UPDATE leave_balance SET {code}_allotted=ROUND(COALESCE({code}_allotted,0)+?,4) WHERE emp_code=? AND year=?",
...
def leave_earn_monthly():
    """Earn Leave (Admin/HR triggered): Admin enters EL & CL days for a Month/Year,
    credited to ALL Active employees. Duplicate-safe (once per month/year) and capped
    to the yearly EL/CL limit set in Payroll Settings."""
    d = request.json or {}
    month = int(d.get("month", date.today().month))
    year  = int(d.get("year",  date.today().year))
    cat   = d.get("category", "")
    remarks = str(d.get("remarks", "")).strip()
    # EL/CL days must be explicitly provided by Admin/HR (validation per Earn Leave popup)
    if d.get("el_days") in (None, "") or d.get("cl_days") in (None, ""):
        return jsonify({"success": False, "error": "Please enter valid EL and CL values."})
    try:
        el_input = float(d.get("el_days"))
        cl_input = float(d.get("cl_days"))
    except (TypeError, ValueError):
        return jsonify({"success": False, "error": "Please enter valid EL and CL values."})
    if el_input < 0 or cl_input < 0:
        return jsonify({"success": False, "error": "EL and CL cannot be negative."})
    ...
    try:
        ...
        for e in emps:
            ec = e["emp_code"]
            # Duplicate-month protection: this Month/Year already processed for this employee
            _already = conn.execute(
                "SELECT id FROM leave_earn_log WHERE emp_code=? AND year=? AND month=? AND leave_type_code=?",
                (ec, year, month, "EL")).fetchone()
            if _already:
                skipped += 1
                continue
            conn.execute("INSERT OR IGNORE INTO leave_balance (emp_code,year) VALUES (?,?)", (ec, year))
            # Yearly cap: never let a single employee earn beyond el_cap/cl_cap in this Financial Year
            eff_el = min(el_input, leave_earn_room(conn, ec, year, "el", el_cap))
            eff_cl = min(cl_input, leave_earn_room(conn, ec, year, "cl", cl_cap)) if e["category"] == "Staff" else 0
            if eff_el <= 0 and eff_cl <= 0:
                capped += 1
                continue
            if e["category"] == "Staff":
                conn.execute("""UPDATE leave_balance SET
                    ...
                    WHERE emp_code=? AND year=?""", (eff_el, eff_cl, ec, year))
                conn.execute("""INSERT OR IGNORE INTO leave_earn_log
                    (emp_code,year,month,el_credited,cl_credited,credited_on,source,leave_type_code,remarks)
                    VALUES (?,?,?,?,?,datetime('now'),'manual','EL',?)""",
                    (ec, year, month, eff_el, eff_cl, remarks))
            else:
                conn.execute("""UPDATE leave_balance SET
                    el_allotted = ROUND(COALESCE(el_allotted,0) + ?, 4)
                    WHERE emp_code=? AND year=?""", (eff_el, ec, year))
                conn.execute("""INSERT OR IGNORE INTO leave_earn_log
                    (emp_code,year,month,el_credited,cl_credited,credited_on,source,leave_type_code,remarks)
                    VALUES (?,?,?,?,?,datetime('now'),'manual','EL',?)""",
...
@app.route("/leaves/year-end-data")
@amgr
def year_end_data():
    """List Active employees with unused EL for a Financial Year, plus their processed status —
    used by the per-employee (selective) Carry Forward / Encash screen."""
    conn = get_db()
    try:
        year = int(request.args.get("year", get_leave_fy_year(conn)))
        emps = conn.execute("""SELECT e.emp_code, e.emp_name, e.department, b.el_allotted, b.el_carried, b.el_used
            FROM employees e LEFT JOIN leave_balance b ON b.emp_code=e.emp_code AND b.year=?
            WHERE e.status='Active' ORDER BY e.emp_name""", (year,)).fetchall()
        done = {r["emp_code"]: dict(r) for r in conn.execute(
            "SELECT emp_code, action, days, amount FROM leave_year_end_log WHERE year=? AND leave_type_code='EL'",
            (year,)).fetchall()}
        rows = []
        for e in emps:
            unused_el = round((e["el_allotted"] or 0) + (e["el_carried"] or 0) - (e["el_used"] or 0), 2)
            rows.append({"emp_code": e["emp_code"], "emp_name": e["emp_name"], "department": e["department"],
                "unused_el": unused_el, "processed": done.get(e["emp_code"])})
        return jsonify({"success": True, "year": year, "employees": rows})
    except Exception as e:
        return jsonify({"success": False, "error": str(e)})
    finally:
        conn.close()

@app.route("/leaves/year-end-selective", methods=["POST"])
@amgr
def year_end_selective():
    """Per-employee Financial-Year-end processing: some employees can be Encashed,
    others Carried Forward, in a single click. EL only (Casual Leave lapses as per policy).
    Duplicate-safe (one action per employee per Financial Year) and runs as one DB transaction."""
    d = request.json or {}
    items = d.get("items", [])
    if not items:
        return jsonify({"success": False, "error": "No employees selected."})
    ...
    processed, errors = [], []
    try:
        year = int(d.get("year", get_leave_fy_year(conn)))
        for item in items:
            ec     = str(item.get("emp_code", "")).strip()
            action = item.get("action", "carry")
            if not ec or action not in ("carry", "encash"):
                continue
            # Duplicate-month/year protection
            already = conn.execute(
                "SELECT id FROM leave_year_end_log WHERE emp_code=? AND year=? AND leave_type_code='EL'",
                (ec, year)).fetchone()
            if already:
                errors.append(f"{ec}: Leave Year-End already processed for FY {year}.")
                continue
            bal = conn.execute("SELECT * FROM leave_balance WHERE emp_code=? AND year=?", (ec, year)).fetchone()
            if not bal:
                errors.append(f"{ec}: No leave balance found for FY {year}.")
                continue
            unused_el = round((bal["el_allotted"] or 0) + (bal["el_carried"] or 0) - (bal["el_used"] or 0), 2)
            if unused_el <= 0:
                errors.append(f"{ec}: No unused EL to process.")
                continue
            days = float(item.get("days", unused_el))
            days = min(days, unused_el)
            if days <= 0:
                continue
            if action == "carry":
                conn.execute("""INSERT INTO leave_balance (emp_code, year, el_carried)
                ...
                conn.execute("""INSERT INTO leave_year_end_log
                    (emp_code,year,leave_type_code,action,days,amount,processed_by,processed_on)
                    VALUES (?,?,'EL','carry',?,0,?,datetime('now'))""",
                    (ec, year, days, session.get("name", "HR")))
                processed.append({"emp_code": ec, "action": "carry", "days": days})
            else:  # encash
                emp = conn.execute("SELECT emp_name, basic FROM employees WHERE emp_code=?", (ec,)).fetchone()
                per_day = float((emp["basic"] if emp else 0) or 0) / 26
                amount = round(per_day * days, 2)
                conn.execute("UPDATE leave_balance SET el_used=el_used+? WHERE emp_code=? AND year=?",
                    (days, ec, year))
                conn.execute("""INSERT INTO leave_requests
                    (emp_code,emp_name,leave_type,from_date,to_date,days,reason,status,
                     applied_on,approved_by,approved_on,source,remarks)
                    VALUES (?,?,'EL',?,?,?,?,'Approved',?,?,datetime('now'),'encashment',?)""",
                    (ec, emp["emp_name"] if emp else "", date.today().isoformat(), date.today().isoformat(),
                     days, f"FY {year} Year-End Encashment", datetime.now().strftime("%Y-%m-%d %H:%M"),
                     session.get("name", "HR"), f"Encashment: Rs.{amount:,.2f} for {days} days."))
                conn.execute("""INSERT INTO leave_year_end_log
                    (emp_code,year,leave_type_code,action,days,amount,processed_by,processed_on)
                    VALUES (?,?,'EL','encash',?,?,?,datetime('now'))""",
...
def my_leaves():
    ...
    requests = conn.execute("""SELECT * FROM leave_requests WHERE emp_code=?
        ORDER BY applied_on DESC LIMIT 100""", (emp_code,)).fetchall()
    bal = conn.execute("SELECT * FROM leave_balance WHERE emp_code=? AND year=?",
                      (emp_code, year)).fetchone()
    emp = conn.execute("SELECT * FROM employees WHERE emp_code=?", (emp_code,)).fetchone()
...
def get_signatures_b64(conn, only_ids=None):
    """Return active named signatures (id, sig_name, img_b64). If only_ids is given
    (list/set of ids as strings or ints), only those are returned, in that order."""
    import base64 as _b64_sig
    rows = conn.execute(
        "SELECT id, sig_name, sig_image, sig_filename FROM letter_signatures WHERE active=1 ORDER BY sig_name"
    ).fetchall()
    by_id = {}
    for r in rows:
        img_b64 = ""
        if r["sig_image"]:
            data = bytes(r["sig_image"])
            if len(data) > 50:
                img_b64 = "data:image/png;base64," + _b64_sig.b64encode(data).decode()
        by_id[str(r["id"])] = {"id": r["id"], "sig_name": r["sig_name"], "img_b64": img_b64}
    if only_ids is None:
...
def letters():
    ...
    templates = conn.execute(
        "SELECT * FROM letter_templates WHERE active=1 ORDER BY template_name"
    ).fetchall()
    pending_docs = conn.execute(
        "SELECT * FROM letter_documents WHERE status='Pending' ORDER BY generated_on DESC"
    ).fetchall()
    approved_docs = conn.execute(
        "SELECT * FROM letter_documents WHERE status='Approved' ORDER BY approved_on DESC LIMIT 10"
    ).fetchall()
    conn.close()
    can_approve = has_permission("letter_approval")
    can_admin = has_permission("letter_admin")
    return render_template("letters.html", employees=emps, has_warning_letter=True,
                           templates=templates, pending_docs=pending_docs, approved_docs=approved_docs,
                           can_approve=can_approve, can_admin=can_admin)


def format_ordinal_date(value):
    """Format a date as '9th July 2026' — used everywhere a date appears on a letter."""
    if value is None:
        return ""
    d = value
    if isinstance(d, str):
        ...
        for fmt in ("%Y-%m-%d", "%d-%m-%Y", "%d/%m/%Y"):
            ...
            return value  # not a recognizable date — leave as-is
...
# ─── LETTER GENERATOR — Custom, admin-editable letter templates ───
def merge_letter_placeholders(body_html, values):
    """Replace {{token}} placeholders in body_html with supplied values.
    Tokens with no supplied value are left untouched (so gaps are visible, not silently dropped) —
    except structural/decorative fields (seal, signatures) which always resolve, blank or not,
    since a raw {{sig_3}} left visible on a printed letter would look like a bug, not a reminder."""
    import re as _re
    def _repl(m):
        key = m.group(1).strip()
        if key in values:
            ...
            if key == "seal" or is_signature_field(key):
                return ""
        return m.group(0)
    return _re.sub(r"\{\{\s*([a-zA-Z0-9_]+)\s*\}\}", _repl, body_html or "")


def extract_letter_tokens(body_html):
    """Return the ordered, de-duplicated list of {{token}} names used in a template body."""
    import re as _re
    seen, out = set(), []
    for m in _re.finditer(r"\{\{\s*([a-zA-Z0-9_]+)\s*\}\}", body_html or ""):
        key = m.group(1).strip()
        if key not in seen:
...
KNOWN_LETTER_FIELDS = {"emp_name","emp_code","department","designation","category","gender","location",
         "date_of_joining","date_of_birth","phone","email","personal_email","official_email",
         "address","aadhar","pan","bank_account","bank_name","ifsc","uan_number","pf_number",
         "esic_number","weekly_off",
         "basic","hra","special_allowance","gross","monthly_ctc","today","company","seal"}

def is_signature_field(key):
    """Signature fields are dynamic tokens like sig_12 (id from letter_signatures)."""
    return bool(re.match(r"^sig_\d+$", key or ""))


@app.route("/letters/templates")
@amgr
def letter_templates_list():
    conn = get_db()
    rows = conn.execute(
        "SELECT * FROM letter_templates WHERE active=1 ORDER BY updated_on DESC, created_on DESC"
    ).fetchall()
    conn.close()
    return render_template("letter_templates.html", templates=rows)


@app.route("/letters/templates/new")
@amgr
def letter_template_new():
    conn = get_db()
    signatures = get_signatures_b64(conn)
    ls = get_letter_settings_b64(conn)
    conn.close()
    return render_template("letter_template_editor.html", tmpl=None, signatures=signatures, ls=ls)


@app.route("/letters/templates/edit/<int:tmpl_id>")
@amgr
def letter_template_edit(tmpl_id):
    conn = get_db()
    row = conn.execute("SELECT * FROM letter_templates WHERE id=?", (tmpl_id,)).fetchone()
    signatures = get_signatures_b64(conn)
    ls = get_letter_settings_b64(conn)
    conn.close()
    if not row:
        return "Template not found", 404
    return render_template("letter_template_editor.html", tmpl=dict(row), signatures=signatures, ls=ls)


@app.route("/letters/templates/save", methods=["POST"])
@amgr
def letter_template_save():
    d = request.json or {}
    name = (d.get("template_name") or "").strip()
    body = d.get("body_html") or ""
    if not name:
        return jsonify({"success": False, "error": "Template name is required"})
    if not body.strip():
        return jsonify({"success": False, "error": "Letter content cannot be empty"})
    tmpl_id = d.get("id")
    doc_prefix = (d.get("doc_code_prefix") or "").strip().upper()[:8]
    format_no = (d.get("format_no") or "").strip()[:60]
    sig_ids = d.get("signature_ids") or []
    sig_ids_str = ",".join(str(int(x)) for x in sig_ids if str(x).strip().isdigit())
    def _mm(key, default):
        try:
            v = float(d.get(key, default))
            return v if 0 <= v <= 60 else default
        except (TypeError, ValueError):
            return default
    m_top    = _mm("margin_top", 15)
    m_right  = _mm("margin_right", 18)
    m_bottom = _mm("margin_bottom", 15)
    m_left   = _mm("margin_left", 18)
    h_margin = _mm("header_margin", 10)
    f_margin = _mm("footer_margin", 10)
    page_size = (d.get("page_size") or "A4").strip().upper()
    if page_size not in ("A4", "A5", "A3", "LEGAL"):
        page_size = "A4"
    now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    conn = get_db()
    try:
        if tmpl_id:
            conn.execute("""UPDATE letter_templates SET
                template_name=?, letter_title=?, body_html=?, show_header=?, show_footer=?,
                doc_code_prefix=?, signature_ids=?, format_no=?,
                margin_top=?, margin_right=?, margin_bottom=?, margin_left=?,
                page_size=?, header_margin=?, footer_margin=?, updated_on=? WHERE id=?""",
                (name, d.get("letter_title", ""), body,
                 1 if d.get("show_header", True) else 0,
                 1 if d.get("show_footer", True) else 0,
                 doc_prefix, sig_ids_str, format_no,
                 m_top, m_right, m_bottom, m_left,
                 page_size, h_margin, f_margin, now, tmpl_id))
            new_id = tmpl_id
        else:
            cur = conn.execute("""INSERT INTO letter_templates
                (template_name, letter_title, body_html, show_header, show_footer,
                 doc_code_prefix, signature_ids, format_no,
                 margin_top, margin_right, margin_bottom, margin_left,
                 page_size, header_margin, footer_margin,
                 created_by, created_on, updated_on, active)
                VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,1)""",
                (name, d.get("letter_title", ""), body,
                 1 if d.get("show_header", True) else 0,
                 1 if d.get("show_footer", True) else 0,
                 doc_prefix, sig_ids_str, format_no,
                 m_top, m_right, m_bottom, m_left,
                 page_size, h_margin, f_margin,
                 session.get("name", "System"), now, now))
...
@app.route("/letters/templates/delete/<int:tmpl_id>", methods=["POST"])
@amgr
def letter_template_delete(tmpl_id):
    conn = get_db()
    try:
        conn.execute("UPDATE letter_templates SET active=0 WHERE id=?", (tmpl_id,))
        conn.commit()
        conn.close()
        return jsonify({"success": True})
    except Exception as e:
...
@app.route("/letters/signatures/list")
@amgr
def letter_signatures_list():
    conn = get_db()
    sigs = get_signatures_b64(conn)
    conn.close()
    return jsonify({"success": True, "signatures": sigs})


@app.route("/letters/signatures/add", methods=["POST"])
@amgr
def letter_signature_add():
    name = (request.form.get("sig_name") or "").strip()
    f = request.files.get("file")
    if not name:
        return jsonify({"success": False, "error": "Signature name is required"})
    if not f:
        return jsonify({"success": False, "error": "Please upload an image"})
    data = f.read()
    conn = get_db()
    try:
        conn.execute("""INSERT INTO letter_signatures (sig_name, sig_image, sig_filename, created_on, active)
            VALUES (?,?,?,?,1)""", (name, data, f.filename, datetime.now().strftime("%Y-%m-%d %H:%M:%S")))
        conn.commit()
        conn.close()
        return jsonify({"success": True})
    except Exception as e:
...
@app.route("/letters/signatures/delete/<int:sig_id>", methods=["POST"])
@amgr
def letter_signature_delete(sig_id):
    conn = get_db()
    try:
        conn.execute("UPDATE letter_signatures SET active=0 WHERE id=?", (sig_id,))
        conn.commit()
        conn.close()
        return jsonify({"success": True})
    except Exception as e:
...
VALID_FIELD_TYPES = {"text", "date", "number"}

@app.route("/letters/field-types/list")
@amgr
def letter_field_types_list():
    conn = get_db()
    rows = conn.execute("SELECT field_key, field_type FROM letter_field_types").fetchall()
    conn.close()
    return jsonify({"field_types": {r["field_key"]: r["field_type"] for r in rows}})


@app.route("/letters/field-types/save", methods=["POST"])
@amgr
def letter_field_types_save():
    d = request.json or {}
    mapping = d.get("field_types") or {}
    now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    conn = get_db()
    try:
        for key, ftype in mapping.items():
            key = (key or "").strip().lower()
            ftype = ftype if ftype in VALID_FIELD_TYPES else "text"
            if not key:
                continue
            conn.execute("""INSERT INTO letter_field_types (field_key, field_type, updated_on)
                VALUES (?,?,?)
                ON CONFLICT(field_key) DO UPDATE SET field_type=excluded.field_type, updated_on=excluded.updated_on""",
                (key, ftype, now))
        conn.commit()
        conn.close()
        return jsonify({"success": True})
...
@app.route("/letters/templates/generate/<int:tmpl_id>")
@amgr
def letter_template_generate_form(tmpl_id):
    conn = get_db()
    tmpl = conn.execute("SELECT * FROM letter_templates WHERE id=?", (tmpl_id,)).fetchone()
    if not tmpl:
        conn.close()
        return "Template not found", 404
    emps = conn.execute(
        "SELECT emp_code, emp_name, department, designation, category, status FROM employees ORDER BY emp_name"
    ).fetchall()
    ftype_rows = conn.execute("SELECT field_key, field_type FROM letter_field_types").fetchall()
    conn.close()
    field_types = {r["field_key"]: r["field_type"] for r in ftype_rows}
    tokens = extract_letter_tokens(tmpl["body_html"])
    custom_tokens = [t for t in tokens if t not in KNOWN_LETTER_FIELDS and not is_signature_field(t)]
    return render_template("letter_template_fill.html", tmpl=dict(tmpl), employees=emps,
                           custom_tokens=custom_tokens, field_types=field_types,
                           today=date.today().strftime("%Y-%m-%d"))


def build_letter_merge_values(conn, form):
    """Builds the {{token}} -> value map from the submitted fill-form: employee fields,
    letter date, company seal, every named signature (image-only), and any custom fields."""
    emp_code = (form.get("emp_code") or "").strip()
    emp = None
    if emp_code:
        emp = conn.execute("SELECT * FROM employees WHERE emp_code=?", (emp_code,)).fetchone()
    ...
    if emp:
        ...
        monthly_ctc = round(base_gross + employer_pf_calc + employer_esi_calc + bonus_calc + gratuity_m, 2)
        values.update({
            "emp_name": emp_d.get("emp_name") or "",
            "emp_code": emp_d.get("emp_code") or "",
            "department": emp_d.get("department") or "",
            "designation": emp_d.get("designation") or "",
            "category": emp_d.get("category") or "",
            "location": emp_d.get("location") or "",
            "gender": emp_d.get("gender") or "",
            "date_of_joining": format_ordinal_date(emp_d.get("date_of_joining")),
            "date_of_birth": format_ordinal_date(emp_d.get("date_of_birth")),
            "phone": emp_d.get("phone") or "",
            "email": emp_d.get("email") or "",
            "personal_email": emp_d.get("personal_email") or "",
            "official_email": emp_d.get("official_email") or "",
            "address": emp_d.get("address") or "",
            "aadhar": emp_d.get("aadhar") or "",
            "pan": emp_d.get("pan") or "",
            "bank_account": emp_d.get("bank_account") or "",
            "bank_name": emp_d.get("bank_name") or "",
            "ifsc": emp_d.get("ifsc") or "",
            "uan_number": emp_d.get("uan_number") or "",
            "pf_number": emp_d.get("pf_number") or "",
            "esic_number": emp_d.get("esic_number") or "",
            "weekly_off": emp_d.get("weekly_off") or "",
            "basic": f"{basic:,.0f}",
            "hra": f"{hra:,.0f}",
            "special_allowance": f"{special:,.0f}",
            "gross": f"{base_gross:,.0f}",
    ...
    if letter_date_raw:
        try:
            letter_date_obj = datetime.strptime(letter_date_raw, "%Y-%m-%d").date()
            values["today"] = format_ordinal_date(letter_date_obj)
        except ValueError:
            pass
    else:
        letter_date_raw = date.today().strftime("%Y-%m-%d")

    # Company seal — inserted only where the template author placed the {{seal}} field
    _ls = get_letter_settings_b64(conn)
    values["seal"] = (f'<img src="{_ls["seal_b64"]}" alt="Seal" style="max-height:80px;max-width:80px;">'
                       if _ls["has_seal"] else "")

    # Every named signature — inserted (image only, no name/company caption) wherever the
    # template author placed its {{sig_<id>}} field.
    for sig in get_signatures_b64(conn):
        values[f"sig_{sig['id']}"] = (f'<img src="{sig["img_b64"]}" alt="{sig["sig_name"]}" style="max-height:55px;">'
                                       if sig.get("img_b64") else "")

    # Any additional custom-field values submitted from the fill form — dates are reformatted
    # to the same "9th July 2026" style used everywhere else on the letter.
    ftype_rows = conn.execute("SELECT field_key, field_type FROM letter_field_types").fetchall()
    field_types = {r["field_key"]: r["field_type"] for r in ftype_rows}
    for k in form.keys():
        if k not in ("emp_code", "letter_date") and k not in values:
            v = form.get(k, "")
            if field_types.get(k) == "date" and v:
                v = format_ordinal_date(v)
            values[k] = v

    return emp_code, values, letter_date_raw


@app.route("/letters/templates/render/<int:tmpl_id>", methods=["POST"])
@amgr
def letter_template_render(tmpl_id):
    """Build the merged letter and submit it for approval.
    No doc code is assigned and nothing is printable until an approver signs off."""
    conn = get_db()
    tmpl = conn.execute("SELECT * FROM letter_templates WHERE id=?", (tmpl_id,)).fetchone()
    if not tmpl:
        conn.close()
    ...
    emp_code, values, letter_date_raw = build_letter_merge_values(conn, request.form)
    merged_body = merge_letter_placeholders(tmpl["body_html"], values)
    now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")

    conn.execute("""INSERT INTO letter_documents
        (template_id, template_name, letter_title, emp_code, emp_name, merged_body,
         show_header, show_footer, signature_ids, format_no,
         margin_top, margin_right, margin_bottom, margin_left, letter_date,
         page_size, header_margin, footer_margin,
         status, generated_by, generated_on)
        VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,'Pending',?,?)""",
        (tmpl_id, tmpl["template_name"], tmpl["letter_title"], emp_code, values.get("emp_name",""),
         merged_body, tmpl["show_header"], tmpl["show_footer"], tmpl["signature_ids"] or "",
         tmpl["format_no"] or "",
         tmpl["margin_top"], tmpl["margin_right"], tmpl["margin_bottom"], tmpl["margin_left"],
         letter_date_raw, tmpl["page_size"] or "A4", tmpl["header_margin"], tmpl["footer_margin"],
         session.get("name","System"), now))
    conn.commit()
    conn.close()

    return render_template("letter_submitted.html", tmpl_name=tmpl["template_name"], emp_name=values.get("emp_name",""))


@app.route("/letters/templates/preview/<int:tmpl_id>", methods=["POST"])
@amgr
def letter_template_preview(tmpl_id):
    """Live preview only — same merge logic as submit, but nothing is saved.
    Used by the fill-form's right-hand live preview panel."""
    conn = get_db()
    tmpl = conn.execute("SELECT * FROM letter_templates WHERE id=?", (tmpl_id,)).fetchone()
    if not tmpl:
        conn.close()
    ...
    _, values, _ = build_letter_merge_values(conn, request.form)
    merged_body = merge_letter_placeholders(tmpl["body_html"], values)
    signatures = get_signatures_b64(conn, (tmpl["signature_ids"] or "").split(","))
    _ls = get_letter_settings_b64(conn)
    conn.close()
    ...
    return render_template("letter_custom_render.html",
        ...
        header_b64=_ls["header_b64"], footer_b64=_ls["footer_b64"],
        margin_top=tmpl["margin_top"] or 15, margin_right=tmpl["margin_right"] or 18,
        margin_bottom=tmpl["margin_bottom"] or 15, margin_left=tmpl["margin_left"] or 18,
        header_margin=tmpl["header_margin"] or 10, footer_margin=tmpl["footer_margin"] or 10,
        page_size=tmpl["page_size"] or "A4", format_no=tmpl["format_no"] or "",
        can_print=False, is_preview=True)




@app.route("/letters/documents/approve/<int:doc_id>", methods=["POST"])
@amgr
def letter_document_approve(doc_id):
    if not has_permission("letter_approval"):
        return jsonify({"success": False, "error": "You do not have permission to approve letters"})
    conn = get_db()
    doc = conn.execute("SELECT * FROM letter_documents WHERE id=?", (doc_id,)).fetchone()
    if not doc:
        conn.close()
        return jsonify({"success": False, "error": "Document not found"})
    if doc["status"] != "Pending":
        conn.close()
        return jsonify({"success": False, "error": f"Document is already {doc['status']}"})

    tmpl = conn.execute("SELECT * FROM letter_templates WHERE id=?", (doc["template_id"],)).fetchone()
    letter_type = f"tmpl{doc['template_id']}"
    prefix = (tmpl["doc_code_prefix"] if tmpl and tmpl["doc_code_prefix"] else "") or None
    if prefix:
        conn.execute("INSERT OR IGNORE INTO letter_counters (letter_type,last_number,prefix) VALUES (?,0,?)", (letter_type, prefix))
        conn.execute("UPDATE letter_counters SET prefix=? WHERE letter_type=?", (prefix, letter_type))
    doc_code = get_next_letter_code(letter_type, conn, doc["emp_code"] or "", doc["emp_name"] or "")

    now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    conn.execute("""UPDATE letter_documents SET status='Approved', doc_code=?, approved_by=?, approved_on=? WHERE id=?""",
                 (doc_code, session.get("name","System"), now, doc_id))
    conn.commit()
    conn.close()
    return jsonify({"success": True, "doc_code": doc_code})


@app.route("/letters/documents/reject/<int:doc_id>", methods=["POST"])
@amgr
def letter_document_reject(doc_id):
    if not has_permission("letter_approval"):
        return jsonify({"success": False, "error": "You do not have permission to reject letters"})
    ...
    if doc["status"] != "Pending":
        ...
        return jsonify({"success": False, "error": f"Document is already {doc['status']}"})
    now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    conn.execute("""UPDATE letter_documents SET status='Rejected', reject_reason=?, approved_by=?, approved_on=? WHERE id=?""",
                 (reason, session.get("name","System"), now, doc_id))
    conn.commit()
    conn.close()
    return jsonify({"success": True})


@app.route("/letters/documents/edit/<int:doc_id>")
@amgr
def letter_document_edit_form(doc_id):
    conn = get_db()
    doc = conn.execute("SELECT * FROM letter_documents WHERE id=?", (doc_id,)).fetchone()
    if not doc:
        conn.close()
    ...
    if doc["status"] != "Pending":
        conn.close()
        return "Only letters that are still Pending approval can be edited.", 403
    edits = conn.execute(
        "SELECT * FROM letter_document_edits WHERE doc_id=? ORDER BY edited_on DESC", (doc_id,)
    ).fetchall()
    conn.close()
    return render_template("letter_document_edit.html", doc=dict(doc), edits=edits)


@app.route("/letters/documents/edit/<int:doc_id>", methods=["POST"])
@amgr
def letter_document_edit_save(doc_id):
    d = request.json or {}
    reason = (d.get("reason") or "").strip()
    body = d.get("merged_body") or ""
    if not reason:
        return jsonify({"success": False, "error": "A reason for this edit is required"})
    if not body.strip():
        return jsonify({"success": False, "error": "Letter content cannot be empty"})
    conn = get_db()
    doc = conn.execute("SELECT * FROM letter_documents WHERE id=?", (doc_id,)).fetchone()
    ...
    if doc["status"] != "Pending":
        conn.close()
        return jsonify({"success": False, "error": f"Document is already {doc['status']} and can no longer be edited"})
    now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    conn.execute("UPDATE letter_documents SET merged_body=? WHERE id=?", (body, doc_id))
    conn.execute("""INSERT INTO letter_document_edits (doc_id, reason, edited_by, edited_on)
                     VALUES (?,?,?,?)""", (doc_id, reason, session.get("name","System"), now))
    conn.commit()
    conn.close()
    return jsonify({"success": True})


@app.route("/letters/documents/view/<int:doc_id>")
@amgr
def letter_document_view(doc_id):
    conn = get_db()
    doc = conn.execute("SELECT * FROM letter_documents WHERE id=?", (doc_id,)).fetchone()
    if not doc:
        conn.close()
    ...
    signatures = get_signatures_b64(conn, (doc["signature_ids"] or "").split(","))
    _ls = get_letter_settings_b64(conn)
    conn.close()
    ...
    return render_template("letter_custom_render.html",
        ...
        header_b64=_ls["header_b64"], footer_b64=_ls["footer_b64"],
        margin_top=doc["margin_top"] or 15, margin_right=doc["margin_right"] or 18,
        margin_bottom=doc["margin_bottom"] or 15, margin_left=doc["margin_left"] or 18,
        header_margin=doc["header_margin"] or 10, footer_margin=doc["footer_margin"] or 10,
        page_size=doc["page_size"] or "A4",
        format_no=doc["format_no"] or "",
        can_print=is_approved, is_preview=False, doc_status=doc["status"])
...
def offer_letter_data():
    ...
    esi = esic_round_up(gross * 0.0075) if emp_d.get("esi_applicable") and gross <= 21000 else 0
    emp_d["gross"] = gross
    emp_d["pf_amount"] = pf
...
def exp_employees():
    ...
    elif status == "Inactive": sql += " AND status!='Active'"
    if cat == "Staff":    sql += " AND category='Staff'"
    elif cat == "NonStaff": sql += " AND category!='Staff'"
    sql += " ORDER BY category,emp_name"
    ...
    thin = Border(**{s:Side(style="thin",color="D0DCF0") for s in ["left","right","top","bottom"]})
    wb = openpyxl.Workbook(); ws = wb.active
    status_label = "" if status=="All" else f" ({status})"
    label = ("Staff Only" if cat=="Staff" else ("Associate Only" if cat=="NonStaff" else "All Employees")) + status_label
    ws.title = label
...
def email_settings():
    conn = get_db()
    # Known SMTP providers — host/port auto-filled; "custom"/"ses"/"other" allow manual host/port entry
    SMTP_PROVIDERS = {
        ...
        "protonmail":   ("127.0.0.1", 1025),   # via ProtonMail Bridge (local)
        "amazon_ses":   None,                   # region-specific — user enters host/port
        "custom":       None,
    }
    if request.method == "POST":
        d = request.form
...
@app.route("/api/wages-data")
@amgr
def api_wages_data():
    """JSON API: Associates OT wages data for payslip wages section"""
    m = int(request.args.get("month", date.today().month))
    y = int(request.args.get("year",  date.today().year))
    conn = get_db()
    try:
        # Get all active associates
        associates = conn.execute("""SELECT emp_code, emp_name, department, designation,
            basic, hra, special_allowance, phone, esic_number
            FROM employees WHERE status='Active' AND category='Associate'
            ORDER BY department, emp_name""").fetchall()

        # Get OT minutes for the month
        ot_rows = conn.execute("""SELECT emp_code, SUM(ot_minutes) as total_ot_min
            FROM attendance
            WHERE strftime('%m',att_date)=? AND strftime('%Y',att_date)=?
            AND ot_minutes > 0
            GROUP BY emp_code""", (f"{m:02d}", str(y))).fetchall()
        ot_by_emp = {r["emp_code"]: (r["total_ot_min"] or 0) for r in ot_rows}

        # Get approved leave records for OT authorization
        approved_rows = conn.execute("""SELECT emp_code, COUNT(*) as cnt
            FROM associate_leave_records
            WHERE strftime('%m', absent_date)=? AND strftime('%Y', absent_date)=?
            AND leave_status='Approved'
            GROUP BY emp_code""", (f"{m:02d}", str(y))).fetchall()
        approved_by_emp = {r["emp_code"]: r["cnt"] for r in approved_rows}

        # Get pending/rejected (unauthorized) days
        unauth_rows = conn.execute("""SELECT emp_code, COUNT(*) as cnt
            FROM attendance
            WHERE strftime('%m',att_date)=? AND strftime('%Y',att_date)=?
            AND status='Absent'
            GROUP BY emp_code""", (f"{m:02d}", str(y))).fetchall()
        absent_by_emp = {r["emp_code"]: r["cnt"] for r in unauth_rows}
        ...
        for e in associates:
            ...
            approved_days = approved_by_emp.get(ec, 0)
            absent_days   = absent_by_emp.get(ec, 0)
            unauth_days   = max(0, absent_days - approved_days)
        ...
        return jsonify({"success": True, "emp_list": emp_list, "month": m, "year": y})
    except Exception as e:
        return jsonify({"success": False, "error": str(e)})
    finally:
        conn.close()
...
@app.route("/wa/send-wages", methods=["POST"])
@amgr
def wa_send_wages():
    """Send WhatsApp wages slip to a single associate"""
    d = request.json
    emp_code = d.get("emp_code","")
    msg      = d.get("message","")
    conn = get_db()
    try:
        emp = conn.execute("SELECT phone, emp_name FROM employees WHERE emp_code=?", (emp_code,)).fetchone()
        if not emp:
            return jsonify({"success":False,"error":"Employee not found"})
        if not emp["phone"]:
            return jsonify({"success":False,"error":f"No phone number for {emp['emp_name']}"})
        ok, err = send_whatsapp(emp["phone"], msg)
        if ok: return jsonify({"success":True})
        else:  return jsonify({"success":False,"error":err})
    finally:
        conn.close()

@app.route("/wa/send-wages-bulk", methods=["POST"])
@amgr
def wa_send_wages_bulk():
    """Send WhatsApp wages slip to multiple associates"""
    d = request.json
    messages  = d.get("messages", [])   # [{emp_code, message}, ...]
    conn = get_db()
    try:
        sent=0; failed=0; no_phone=0
        for item in messages:
            ec  = item.get("emp_code","")
            msg = item.get("message","")
            emp = conn.execute("SELECT phone, emp_name FROM employees WHERE emp_code=?", (ec,)).fetchone()
...
# ─── ASSOCIATE OT LEAVE (Dept Head Apply → HR Approve → OT Authorized) ────

@app.route("/assoc-leave")
@amgr
def assoc_leave_index():
    role = session.get("role","")
    perms = session.get("permissions",[])
    if role == "admin" or "assoc_leave_approve" in perms or "leave_assoc" in perms:
        return redirect("/assoc-leave/approve")
    return redirect("/assoc-leave/apply")

@app.route("/assoc-leave/apply")
@amgr
def assoc_leave_apply_page():
    """Dept Head: submit associate absent dates for OT authorization"""
    # Module removed from navigation.
    return redirect("/dashboard")
    conn = get_db()
    try:
        ...
        # Get dept head's departments using correct table
        if role == "admin":
            dept_rows = conn.execute(
                "SELECT DISTINCT department FROM employees WHERE status='Active' AND category='Associate' ORDER BY department"
            ).fetchall()
            my_depts = [r["department"] for r in dept_rows]
        else:
            u = conn.execute("SELECT id FROM users WHERE username=?", (user_name,)).fetchone()
            if u:
                dept_rows = conn.execute(
                    "SELECT department FROM dept_head_assignments WHERE user_id=? ORDER BY department",
                    (u["id"],)).fetchall()
                my_depts = [r["department"] for r in dept_rows]
            else:
                my_depts = []
        ...
        if dept_filter and dept_filter in my_depts:
            show_dept = dept_filter
        else:
            show_dept = my_depts[0] if my_depts else ""

        # Get associates in selected dept
        associates = []
        if show_dept:
            associates = conn.execute(
                "SELECT emp_code, emp_name, department FROM employees WHERE status='Active' AND category='Associate' AND department=? ORDER BY emp_name",
                (show_dept,)
            ).fetchall()

        # Existing requests (last 90 days)
        from_dt = (date.today() - timedelta(days=90)).strftime("%Y-%m-%d")
        if my_depts:
            ph = ",".join("?"*len(my_depts))
            existing = conn.execute(f"""
                SELECT r.*, e.department FROM associate_ot_leave_requests r
                JOIN employees e ON r.emp_code=e.emp_code
                WHERE e.department IN ({ph})
                AND r.from_date >= ?
                ORDER BY r.from_date DESC, r.emp_name
            """, my_depts + [from_dt]).fetchall()
        else:
            existing = []

        return render_template("assoc_leave_apply.html",
            my_depts=my_depts, show_dept=show_dept,
            associates=[dict(a) for a in associates],
            existing=[dict(r) for r in existing],
            today=date.today().strftime("%Y-%m-%d"))
    except Exception as e:
        return f"<h3>Error: {e}</h3><a href='/'>← Back</a>", 500
    finally:
        conn.close()

@app.route("/assoc-leave/apply", methods=["POST"])
@amgr
def assoc_leave_apply_post():
    """Dept Head submits associate absent date range for OT authorization"""
    d = request.json
    conn = get_db()
    try:
        emp_code  = d.get("emp_code","").strip()
        from_date = d.get("from_date","").strip()
        to_date   = d.get("to_date","").strip() or from_date
        reason    = d.get("reason","").strip()

        if not emp_code or not from_date:
            return jsonify({"success":False,"error":"Employee and date required"})
        if not reason:
            return jsonify({"success":False,"error":"Reason is required"})

        emp = conn.execute(
            "SELECT * FROM employees WHERE emp_code=? AND category='Associate' AND status='Active'",
            (emp_code,)).fetchone()
        if not emp:
            return jsonify({"success":False,"error":"Employee not found or not an Associate"})
        ...
        for yr in years:
            rows = conn.execute(
                ...
                (f"{yr}%",)).fetchall()
            for r in rows:
                holiday_set.add(r["holiday_date"])

        # Weekly off for associate (default Sunday = 6)
        wo_str = emp["weekly_off"] if "weekly_off" in emp.keys() and emp["weekly_off"] else "Sunday"
        _wo_map = {"Monday":0,"Tuesday":1,"Wednesday":2,"Thursday":3,
                   "Friday":4,"Saturday":5,"Sunday":6}
        wo_weekday = _wo_map.get(wo_str, 6)

        # Count working days (skip weekly off + holidays)
        working_days = 0
        cur = fd
        while cur <= td:
            ds = cur.strftime("%Y-%m-%d")
        ...
        if working_days == 0:
            return jsonify({"success":False,
                "error":"No working days in selected range (all are weekly off or holidays)."})

        # Check duplicate — Approved/Pending pe block, Rejected pe allow
        existing = conn.execute("""
            SELECT id, status FROM associate_ot_leave_requests
            WHERE emp_code=? AND from_date=? AND to_date=?
        ...
        if existing:
            ...
            if st == 'Pending':
                return jsonify({"success":False,
                    "error":f"Request already Pending for {emp['emp_name']} ({from_date} → {to_date}). Please wait for approval."})
            else:
                return jsonify({"success":False,
                    "error":f"Request already Approved for {emp['emp_name']} ({from_date} → {to_date})."})
        # Rejected wali request → allow new submission (delete old rejected row first)
        conn.execute("""DELETE FROM associate_ot_leave_requests
            WHERE emp_code=? AND from_date=? AND to_date=? AND status='Rejected'""",
            (emp_code, from_date, to_date))

        conn.execute("""INSERT INTO associate_ot_leave_requests
            (emp_code, emp_name, department, from_date, to_date, absent_date,
             month, year, reason, status, submitted_by, submitted_on, working_days)
            VALUES (?,?,?,?,?,?,?,?,?,'Pending',?,datetime('now'),?)""",
            (emp_code, emp["emp_name"], emp["department"],
             from_date, to_date, from_date,
             fd.month, fd.year,
             reason, session.get("name","DeptHead"), working_days))
        conn.commit()

        total_days = (td - fd).days + 1
        skipped = total_days - working_days
        msg = f"Request submitted for {emp['emp_name']} — {working_days} working day(s) ({from_date} → {to_date})"
        if skipped:
            msg += f", {skipped} day(s) skipped (weekly off/holidays)"
        return jsonify({"success":True,"message":msg})
    except Exception as e:
        return jsonify({"success":False,"error":str(e)})
    finally:
...
@app.route("/assoc-leave/approve")
@amgr
def assoc_leave_approval_page():
    """HR/Admin: Approve associate OT authorization requests"""
    # Module removed from navigation.
    return redirect("/dashboard")
    conn = get_db()
    try:
        status_filter = request.args.get("status","Pending")
        dept_filter   = request.args.get("dept","")
        month_filter  = int(request.args.get("month", date.today().month))
        year_filter   = int(request.args.get("year",  date.today().year))

        sql = """SELECT r.*, e.department
            FROM associate_ot_leave_requests r
            JOIN employees e ON r.emp_code=e.emp_code
            WHERE 1=1"""
        params = []
        ...
        if dept_filter:
            sql += " AND e.department=?"; params.append(dept_filter)
        if month_filter:
            sql += " AND r.month=?"; params.append(month_filter)
        if year_filter:
            sql += " AND r.year=?"; params.append(year_filter)
        sql += " ORDER BY r.from_date DESC, r.emp_name"
        ...
        pending_count = conn.execute(
            "SELECT COUNT(*) as c FROM associate_ot_leave_requests WHERE status='Pending'"
        ).fetchone()["c"]

        return render_template("assoc_leave_approve.html",
            requests=[dict(r) for r in requests_list],
            depts=[d["department"] for d in depts],
            status_filter=status_filter, dept_filter=dept_filter,
            month_filter=month_filter, year_filter=year_filter,
            pending_count=pending_count, months=MONTHS,
            today=date.today().strftime("%Y-%m-%d"))
    finally:
        conn.close()

@app.route("/assoc-leave/approve", methods=["POST"])
@amgr
def assoc_leave_approve_post():
    """HR/Admin approves or rejects associate OT leave requests"""
    d = request.json
    conn = get_db()
    try:
        ids     = d.get("ids", [])
        ...
        remarks = d.get("remarks","")
        if not ids:
            return jsonify({"success":False,"error":"No requests selected"})
        ...
        for req_id in ids:
            req = conn.execute(
                "SELECT * FROM associate_ot_leave_requests WHERE id=?", (req_id,)
            ).fetchone()
            if not req:
                continue

            conn.execute("""UPDATE associate_ot_leave_requests
                SET status=?, approved_by=?, approved_on=?, remarks=?
                WHERE id=?""", (action, by, now, remarks, req_id))

            # If Approved → sync each working day in range to associate_leave_records
            if action == "Approved":
                leave_from = req["from_date"] or req["absent_date"] or ""
                leave_to   = req["to_date"]   or leave_from
                if leave_from:
                    # Get holidays
                    yr = int(leave_from[:4])
                    hols = conn.execute(
                    ...
                    emp_row = conn.execute(
                        "SELECT weekly_off FROM employees WHERE emp_code=?",
                        (req["emp_code"],)).fetchone()
                    wo_str = (emp_row["weekly_off"] if emp_row and emp_row["weekly_off"] else "Sunday")
                    _wo_map = {"Monday":0,"Tuesday":1,"Wednesday":2,"Thursday":3,
                               "Friday":4,"Saturday":5,"Sunday":6}
                    wo_weekday = _wo_map.get(wo_str, 6)

                    fd2 = datetime.strptime(leave_from, "%Y-%m-%d").date()
                    td2 = datetime.strptime(leave_to,   "%Y-%m-%d").date()
                    cur2 = fd2
                    while cur2 <= td2:
                        ds2 = cur2.strftime("%Y-%m-%d")
                        if cur2.weekday() != wo_weekday and ds2 not in holiday_set:
                            conn.execute("""INSERT OR REPLACE INTO associate_leave_records
                                (emp_code, emp_name, month, year, absent_date,
                                 leave_status, approved_by, approved_on, remarks)
                                VALUES (?,?,?,?,?,'Approved',?,?,?)""",
                                (req["emp_code"], req["emp_name"],
                                 cur2.month, cur2.year, ds2,
                                 by, now, remarks or "OT Authorized via Approval Center"))
                            approved_count += 1
                ...
                if leave_from:
                    ...
                    cur2 = fd2
                    while cur2 <= td2:
                        conn.execute("DELETE FROM associate_leave_records WHERE emp_code=? AND absent_date=?",
                            (req["emp_code"], cur2.strftime("%Y-%m-%d")))
                        cur2 += timedelta(days=1)

        conn.commit()
        msg = f"{len(ids)} request(s) {action}"
        if action == "Approved":
            msg += f" — {approved_count} OT date(s) now Authorized"
        return jsonify({"success":True,"message":msg})
    except Exception as e:
        return jsonify({"success":False,"error":str(e)})
    finally:
...
def generate_punch_alerts():
    """
    Punch Alert Logic — VPL RD HRMS & Payroll Solution
    ═══════════════════════════════════════════════════════
    ...
    try:
        ...
        for lt in earn_types:
            ...
            for e in emps:
                ...
                # Yearly Financial-Year cap: never let EL exceed el_cap or CL exceed cl_cap per employee/year
                if lt_code == "el":
                    eff_earn = min(eff_earn, leave_earn_room(conn, ec, year, "el", el_cap))
                elif lt_code == "cl":
                    eff_earn = min(eff_earn, leave_earn_room(conn, ec, year, "cl", cl_cap))
                if eff_earn <= 0: continue
                try:
...
def salary_revision_report():
    ...
    return render_template("salary_revision.html", revisions=revisions, months=MONTHS, date=_dt_rev, designations=designations, is_admin_page=_is_admin)


@app.route("/salary/increment/edit/<int:inc_id>", methods=["POST"])
def edit_increment(inc_id):
    """Edit an existing increment record — admin only for this action"""
    if session.get("role") != "admin":
        return jsonify({"success": False, "error": "Access denied. Admin only."})
    d = request.json or {}
    conn = get_db()
    try:
        inc = conn.execute("SELECT * FROM increments WHERE id=?", (inc_id,)).fetchone()
        if not inc:
            conn.close()
            return jsonify({"success": False, "error": "Record not found"})
        ...
        do_salary = d.get("salary_change", True)
        do_desig  = d.get("designation_change", True)

        old_basic      = float(d.get("old_basic") or inc["old_basic"])
        effective_date = str(d.get("effective_date") or inc["effective_date"])
        new_desig      = str(d.get("new_designation") or inc["new_designation"] or "") if do_desig else (inc["new_designation"] or "")
        old_desig      = str(d.get("old_designation") or inc["old_designation"] or "")

        # Salary: use override_gross → split 60/40, then allow override_basic/hra on top
        if do_salary:
            override_gross = d.get("override_gross")
            override_hra   = d.get("override_hra")
            new_basic_raw  = d.get("new_basic")

            if override_gross:
                gross = round(float(override_gross), 2)
                new_basic = round(float(new_basic_raw), 2) if new_basic_raw else round(gross * 0.60, 2)
                new_hra   = round(float(override_hra), 2)  if override_hra  else round(gross * 0.40, 2)
                # Validate basic + hra ≤ gross
                if new_basic + new_hra > gross + 0.02:
                    conn.close()
                    return jsonify({"success": False,
                        "error": f"Basic (₹{new_basic:,.0f}) + HRA (₹{new_hra:,.0f}) = ₹{new_basic+new_hra:,.0f} exceeds New Gross ₹{gross:,.0f}."})
            elif new_basic_raw:
                new_basic = round(float(new_basic_raw), 2)
                new_hra   = round(float(override_hra), 2) if override_hra else round(new_basic * 0.40, 2)
            else:
                new_basic = float(inc["new_basic"])
                new_hra   = round(new_basic * 0.40, 2)
        else:
            new_basic = float(inc["new_basic"])
            new_hra   = None

        conn.execute("""UPDATE increments SET old_basic=?,new_basic=?,effective_date=?,reason=?,old_designation=?,new_designation=? WHERE id=?""",
            (old_basic, new_basic, effective_date, reason, old_desig, new_desig, inc_id))

        # If this is the latest record, update employee master too
        latest = conn.execute("SELECT id FROM increments WHERE emp_code=? ORDER BY effective_date DESC, id DESC LIMIT 1",(inc["emp_code"],)).fetchone()
        if latest and latest["id"] == inc_id:
            if do_salary:
                if new_hra is not None:
                    conn.execute("UPDATE employees SET basic=?,hra=? WHERE emp_code=?",(new_basic, new_hra, inc["emp_code"]))
                else:
                    conn.execute("UPDATE employees SET basic=? WHERE emp_code=?",(new_basic, inc["emp_code"]))
            if do_desig and new_desig:
                conn.execute("UPDATE employees SET designation=? WHERE emp_code=?",(new_desig, inc["emp_code"]))
...
def salary_history(emp_code):
    ...
    designations = [r["name"] for r in conn.execute("SELECT name FROM master_designations WHERE is_active=1 ORDER BY name").fetchall()]
    conn.close()
    return render_template("salary_history.html",emp=dict(emp),increments=increments,records=records,months=MONTHS,designations=designations)
            ...
            for uid_raw, punches in emp_punches_all.items():
                ...
                for pair in pairs:
                    ...
                    try:
                        ...
                        auto_status = "WOP" if att_d2.weekday()==get_emp_weekly_off_num(ec, conn, on_date=att_d2) else "Present"
                        if auto_status == "Present":
...
def exp_att_employee(emp_code, m, y):
    ...
    for day in range(1, month_days+1):
        ...
        if not status:
            if d.weekday() == get_emp_weekly_off_num(emp_code, on_date=d):
                status = "WO"
            else:
...
@app.route("/weekly-off/recalculate", methods=["POST"])
@amgr
def weekly_off_recalculate():
    """
    Recalculate attendance WO/WOP/Absent status for a single employee,
    from a given start date through the current month, using their
    (date-aware) Weekly Off assignments. Used by the Weekly Off Management
    module's 'Recalculate' action, so attendance immediately reflects a
    freshly assigned Weekly Off without waiting for the next auto-import.
    """
    d = request.json or {}
    emp_code  = (d.get("emp_code") or "").strip()
    from_date = (d.get("from_date") or "").strip()
    if not emp_code:
        return jsonify({"success": False, "error": "Employee required."})
    ...
    try:
        ...
        while (y, m) <= (today.year, today.month):
            hols = {h["holiday_date"] for h in conn.execute(
                """SELECT holiday_date FROM holidays
                   WHERE strftime('%m',holiday_date)=? AND strftime('%Y',holiday_date)=?""",
                (f"{m:02d}", str(y))).fetchall()}
            fixed += _fix_weekly_off_for_emp_month(conn, emp_code, m, y, hols)
            m += 1
            if m > 12:
                m = 1; y += 1
        ...
        return jsonify({"success": True, "fixed": fixed,
                        "message": f"Attendance recalculated ({fixed} record(s) updated)."})
    except Exception as e:
        return jsonify({"success": False, "error": str(e)})
    finally:
        conn.close()
...
@app.route("/weekly-off")
@amgr
def weekly_off_module():
    conn = get_db()
    sync_weekly_off_column(conn)  # refresh today's effective values before showing
    today_str = date.today().isoformat()

    employees = conn.execute(
        "SELECT emp_code,emp_name,department,category,COALESCE(weekly_off,'Sunday') as weekly_off "
        "FROM employees WHERE status='Active' ORDER BY emp_name").fetchall()

    # Distinct Department / Category lists, for the filter dropdowns
    departments = [r["department"] for r in conn.execute(
        "SELECT DISTINCT department FROM employees WHERE status='Active' AND department IS NOT NULL AND department!='' ORDER BY department").fetchall()]
    categories = [r["category"] for r in conn.execute(
        "SELECT DISTINCT category FROM employees WHERE status='Active' AND category IS NOT NULL AND category!='' ORDER BY category").fetchall()]

    # All assignment records, joined with employee name, newest first.
    # We flag each as active / upcoming / expired so the template can sort
    # active & upcoming (still relevant) to the top, expired ones below.
    rows = conn.execute("""SELECT wh.*, COALESCE(e.emp_name,'Unknown') as emp_name,
            COALESCE(e.department,'—') as department, COALESCE(e.category,'—') as category
        FROM weekly_off_history wh
        LEFT JOIN employees e ON wh.emp_code=e.emp_code
        WHERE wh.created_by IS NULL OR wh.created_by != 'system_migration'
        ORDER BY wh.from_date DESC, wh.id DESC""").fetchall()

    assignments = []
    for r in rows:
        d = dict(r)
        if d["to_date"] and d["to_date"] < today_str:
            d["status"] = "expired"
        elif d["from_date"] > today_str:
            d["status"] = "upcoming"
        else:
            d["status"] = "active"
        assignments.append(d)

    # Active/upcoming shown first, expired below
    status_order = {"active": 0, "upcoming": 1, "expired": 2}
    assignments.sort(key=lambda x: (status_order.get(x["status"], 3), x["from_date"]), reverse=True)
    assignments.sort(key=lambda x: status_order.get(x["status"], 3))

    conn.close()

    role = session.get("role", "")
    perms = set(session.get("permissions", []) or [])
    can_bulk_upload = (role == "admin") or ("weekly_off_bulk_upload" in perms)

    return render_template("weekly_off.html", employees=employees, assignments=assignments,
                            today=today_str, departments=departments, categories=categories,
                            can_bulk_upload=can_bulk_upload)


def _assign_weekly_off_row(conn, emp_code, wo_day, from_date, to_date, created_by):
    """Create one dated Weekly Off assignment for an employee. Shared by the
    single Assign action and the Bulk Upload importer. Returns (True, None)
    on success or (False, error_message) on failure.
    """
    valid_days = ["Monday","Tuesday","Wednesday","Thursday","Friday","Saturday","Sunday"]
    if not emp_code or wo_day not in valid_days or not from_date:
        return False, "Employee, Weekly Off day and From Date are required."
    if to_date and to_date < from_date:
        return False, "To Date cannot be before From Date."

    emp = conn.execute("SELECT emp_code FROM employees WHERE emp_code=?", (emp_code,)).fetchone()
    if not emp:
        return False, f"Employee {emp_code} not found."

    # Close any still-open (to_date IS NULL) prior assignment for this
    # employee so it no longer applies from this new change onward —
    # past dates before `from_date` remain governed by that old record.
    conn.execute("""UPDATE weekly_off_history SET to_date=?
                     WHERE emp_code=? AND to_date IS NULL AND from_date<?""",
                 (( _dtsub1(from_date) ), emp_code, from_date))

    conn.execute("""INSERT INTO weekly_off_history
            (emp_code, weekly_off_day, from_date, to_date, created_by)
            VALUES (?,?,?,?,?)""",
                 (emp_code, wo_day, from_date, to_date, created_by))
    return True, None


@app.route("/weekly-off/assign", methods=["POST"])
@amgr
def weekly_off_assign():
    """Assign a dated Weekly Off to an employee.
    Only dates >= from_date are affected; past dates keep whatever weekly off
    was in effect earlier. If to_date is given and later passes, the employee
    automatically reverts to the default (Sunday) once no assignment covers
    that date anymore.
    """
    d = request.json or {}
    emp_code   = (d.get("emp_code") or "").strip()
    wo_day     = (d.get("weekly_off_day") or "").strip()
    from_date  = (d.get("from_date") or "").strip()
    to_date    = (d.get("to_date") or "").strip() or None

    conn = get_db()
    try:
        ok, err = _assign_weekly_off_row(conn, emp_code, wo_day, from_date, to_date,
                                          session.get("name", "HR"))
        if not ok:
            return jsonify({"success": False, "error": err})
        conn.commit()
        sync_weekly_off_column(conn)
        return jsonify({"success": True})
    except Exception as e:
        return jsonify({"success": False, "error": str(e)})
    finally:
...
def _dtsub1(date_str):
    """Return date_str minus 1 day, as YYYY-MM-DD string."""
    try:
...
@app.route("/weekly-off/delete/<int:wid>", methods=["POST"])
@amgr
def weekly_off_delete(wid):
    """Cancel/remove a Weekly Off assignment record."""
    conn = get_db()
    try:
        conn.execute("DELETE FROM weekly_off_history WHERE id=?", (wid,))
        conn.commit()
        sync_weekly_off_column(conn)
        return jsonify({"success": True})
    except Exception as e:
...
@app.route("/weekly-off/download-template")
@amgr
def weekly_off_download_template():
    """Download the Bulk Upload Excel template for Weekly Off assignments."""
    role = session.get("role", "")
    perms = set(session.get("permissions", []) or [])
    if not (role == "admin" or "weekly_off_bulk_upload" in perms):
        return jsonify({"success": False, "error": "You do not have permission for Weekly Off Bulk Upload."}), 403
    ...
    wb = openpyxl.Workbook()
    ws = wb.active
    ws.title = "Weekly Off Bulk Upload"

    hdrs  = ["Employee Code", "Day", "From Date", "To Date"]
    notes = ["Employee Code (as in Employee Profile)",
              "Weekly Off Day (Monday/Tuesday/.../Sunday)",
              "From Date (YYYY-MM-DD or DD-MM-YYYY)",
              "To Date — optional, leave blank for an ongoing assignment"]
    for ci, (h, n) in enumerate(zip(hdrs, notes), 1):
        cell = ws.cell(row=1, column=ci, value=h)
        cell.font = Font(bold=True, color="FFFFFF")
        cell.fill = PatternFill("solid", fgColor="0052CC")
        cell.alignment = Alignment(horizontal="center")
        nc = ws.cell(row=2, column=ci, value=n)
        nc.font = Font(italic=True, color="888888", size=9)

    samples = [
        ["1001", "Tuesday",  "2026-08-01", ""],
        ["1002", "Wednesday","2026-08-01", "2026-12-31"],
    ]
    for ri, row in enumerate(samples, 3):
        for ci, val in enumerate(row, 1):
    ...
    out = io.BytesIO(); wb.save(out); out.seek(0)
    return send_file(out, mimetype="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                     as_attachment=True, download_name="WeeklyOff_BulkUpload_Template.xlsx")


@app.route("/weekly-off/bulk-upload", methods=["POST"])
@amgr
def weekly_off_bulk_upload():
    """Bulk-assign Weekly Off from an uploaded Excel file.
    Expected columns: Employee Code, Day, From Date, To Date (To Date optional).
    Requires the dedicated 'Weekly Off Bulk Upload' permission (or admin).
    """
    role = session.get("role", "")
    perms = set(session.get("permissions", []) or [])
    if not (role == "admin" or "weekly_off_bulk_upload" in perms):
        return jsonify({"success": False, "error": "You do not have permission for Weekly Off Bulk Upload."})
    ...
    try:
        ...
        return jsonify({"success": False, "error": f"Could not read the Excel file: {e}"})
    ...
    hdrs = [_norm_header(c.value) for c in ws[1]]
    valid_days = ["Monday","Tuesday","Wednesday","Thursday","Friday","Saturday","Sunday"]
    ...
    try:
        for rn, row in enumerate(ws.iter_rows(min_row=2, values_only=True), start=2):
            if not any(row):
                continue
            d = dict(zip(hdrs, row))
            raw_code = d.get("employee_code") or d.get("emp_code") or d.get("employee") or ""
            if raw_code is None: raw_code = ""
            emp_code = str(int(float(str(raw_code)))).strip() if str(raw_code).strip().replace(".", "").isdigit() else str(raw_code).strip()

            wo_raw = str(d.get("day") or d.get("weekly_off_day") or d.get("weekly_off") or "").strip()
            wo_day = next((day for day in valid_days if day.lower() == wo_raw.lower()), None)

            from_date = _parse_date(d.get("from_date"))
            to_date   = _parse_date(d.get("to_date"))

            if not emp_code:
                continue  # skip blank rows
            if not wo_day:
                errors.append(f"Row {rn}: invalid Day '{wo_raw}'"); continue
            if not from_date:
                errors.append(f"Row {rn}: From Date missing/invalid"); continue

            ok, err = _assign_weekly_off_row(conn, emp_code, wo_day, from_date, to_date,
                                              session.get("name", "HR") + " (Bulk Upload)")
            if ok:
                added += 1
                affected.add((emp_code, from_date))
            else:
                errors.append(f"Row {rn} ({emp_code}): {err}")

        conn.commit()
        sync_weekly_off_column(conn)

        # Recalculate attendance for every employee affected by this upload,
        # from their new assignment's from_date through the current month.
        fixed_total = 0
        for emp_code, from_date in affected:
            try:
                today = date.today()
                ...
                while (y, m) <= (today.year, today.month):
                    key = (y, m)
                    if key not in hols_cache:
                        hols_cache[key] = {h["holiday_date"] for h in conn.execute(
                            """SELECT holiday_date FROM holidays
                               WHERE strftime('%m',holiday_date)=? AND strftime('%Y',holiday_date)=?""",
                            (f"{m:02d}", str(y))).fetchall()}
                    fixed_total += _fix_weekly_off_for_emp_month(conn, emp_code, m, y, hols_cache[key])
                    m += 1
                    if m > 12:
                        m = 1; y += 1
            except Exception:
        ...
        return jsonify({"success": True, "added": added, "errors": errors,
                        "message": f"{added} assignment(s) created, {fixed_total} attendance record(s) recalculated."
        ...
        for emp in emps:
            ...
            for att_date_r in iter_dates:
                try:
                    ...
                    is_sun = (d2.weekday() == get_emp_weekly_off_num(emp["emp_code"], on_date=d2))
                except:
...
def att_register_report():
    ...
    def make_sheet_detailed(emp, m, y):
        ...
        for day in range(1, days_in_month+1):
            ...
            is_sun = day_obj.weekday() == get_emp_weekly_off_num(emp["emp_code"], on_date=day_obj)
...
def _fix_weekly_off_for_emp_month(conn, ec, month, year, hols):
    """Correct WO/WOP/Absent status for one employee for one month, based on
    the (date-aware) Weekly Off assignment in effect for each day. Returns
    the number of attendance rows fixed. Shared by the global 'Fix Weekly
    Off' tool (Time & Attendance page) and the per-employee 'Recalculate'
    action in the Weekly Off Management module.
    """
    import calendar as _cal
    from datetime import date as _dt
    ...
    for day in range(1, total_days + 1):
        ...
        dt_str = d_obj.strftime("%Y-%m-%d")
        is_wo_day = (d_obj.weekday() == get_emp_weekly_off_num(ec, conn, on_date=d_obj))
...
def fix_weekly_off():
    ...
    try:
        ...
        fixed = 0
        for emp in emps:
            fixed += _fix_weekly_off_for_emp_month(conn, emp["emp_code"], month, year, hols)
...
def recalculate_attendance():
                                ...
                                if _prev_out < _prev_in:  # cross-midnight shift
                                    ...
                                    if _prev_out_next == in_dt_candidate:
                                        ...
                                        try:
                                            ...
                                            _wo_num2  = get_emp_weekly_off_num(ec, conn, on_date=att_d)
                                ...
                                if _prev_in2_min >= 17*60 and _today_in_min <= 10*60:
                                    ...
                                    if _is_ns or _prev_in2_min >= 17*60:  # night shift or late IN
                                        ...
                                        try:
                                            ...
                                            _wo_num_chk = get_emp_weekly_off_num(ec, conn, on_date=att_d)
        ...
        for pair in pairs:
            ...
            try:
                ...
                if preserved_status:
                    ...
                    status_ovrd = None
                elif att_dt.weekday() == get_emp_weekly_off_num(ec, conn, on_date=att_dt):
                    auto_status = "WOP"
                    status_ovrd = "WOP"
                else:
                    hol = conn.execute("""SELECT id FROM holidays WHERE holiday_date=?
    ...
    for emp in emps:
        ...
        while cur <= td:
            ...
            is_sun = (cur.weekday() == get_emp_weekly_off_num(ec, on_date=cur))
...
def my_attendance():
    ...
    # Get approved leaves for this month — taaki attendance portal mein "Leave" show ho
    import calendar as _cal_my
    _days_in_m = _cal_my.monthrange(year, month)[1]
    _m_start = f"{year}-{month:02d}-01"
    _m_end   = f"{year}-{month:02d}-{_days_in_m:02d}"
    leave_rows = conn.execute("""
        SELECT from_date, to_date, is_half_day, leave_type
        FROM leave_requests
        WHERE emp_code=? AND status='Approved'
          AND from_date <= ? AND to_date >= ?
    """, (emp_code, _m_end, _m_start)).fetchall()

    # Build leave_dates map: date -> {is_half_day, leave_type}
    from datetime import timedelta as _td_my
    leave_date_map = {}
    for lr in leave_rows:
        try:
            ld_start = date.fromisoformat(lr["from_date"])
            ld_end   = date.fromisoformat(lr["to_date"])
            cur = ld_start
            while cur <= ld_end:
                leave_date_map[cur.strftime("%Y-%m-%d")] = {
                    "is_half_day": lr["is_half_day"] or 0,
                    "leave_type": lr["leave_type"] or "Leave"
                }
                cur += _td_my(days=1)
        except: pass
    ...
    # Get free late days setting
    try:
        _conn_ps = get_db()
        _ps = get_payroll_settings(_conn_ps)
        _late_free_days = int(_ps.get("late_free_days", 2) or 2)
        _conn_ps.close()
    except:
        _late_free_days = 2
    ...
    attendance_rows = []
    summary = {"present":0,"absent":0,"leave":0,"half_day":0,"holiday":0,"total_hours":0.0}
    _running_late_count = 0  # serial late counter for colour coding
    ...
    for d_num in range(1, days_in_month+1):
        ...
        # ── Leave override: agar approved leave hai aur att status Half Day/Absent hai
        # toh employee portal pe "Leave" dikhao
        _leave_info = leave_date_map.get(dt_str)
        if _leave_info:
            _is_hd_leave = _leave_info["is_half_day"]
            if _is_hd_leave:
                # Half Day Leave: half_day flag set + late ALWAYS 0 (employee 2nd half se aaya)
                half_day = 1
                late_min = 0  # Half day leave approved → late mark kabhi nahi
                # Status override for employee portal display
                if status in ("Absent", "Miss Punch", ""):
                    status = "Leave"
            else:
                # Full Day Leave
                status = "Leave"
        ...
        # ── Late serial count for colour coding ───────────────────
        # is_halfday_from_late: IN time > 12:30 → half day (no late_min, but is_half_day=1)
        _is_halfday_from_late = False
        _late_serial = 0
        ...
        if late_min and late_min > 0:
            ...
            try:
                _in_parts = in_time.split(":")
                _in_mins = int(_in_parts[0]) * 60 + int(_in_parts[1])
                if _in_mins > 12 * 60 + 30:
                    _is_halfday_from_late = True
            except: pass
        ...
        attendance_rows.append({
            ...
            "late_serial": _late_serial,
            "late_is_deducted": _late_is_deducted,
...
def dept_head_pending():
    ...
    # Get departments this user heads — admins implicitly head every department, no assignment needed
    is_admin = session.get("role") == "admin"
    if is_admin:
        my_depts = [r["department"] for r in conn.execute(
            "SELECT DISTINCT department FROM employees WHERE department IS NOT NULL AND department!=''").fetchall()]
...
def dept_head_approve(leave_id):
    ...
    try:
        ...
        # Verify this user heads the employee's department — admins are implicitly head of every department
                        ...
                        try:
                            ...
                            for pair in pairs_a:
                                ...
                                try:
                                    ...
                                    auto_status = "WOP" if att_d2.weekday()==get_emp_weekly_off_num(ec_a, on_date=att_d2) else "Present"
                                except: auto_status = "Present"
            ...
            try:
                ...
                if existing:
                    ...
                    try:
                        ...
                        if att_dt.weekday() == get_emp_weekly_off_num(ec, on_date=att_dt):
                            auto_status = "WOP"
                        else:
                            hol = conn.execute("""SELECT id FROM holidays WHERE holiday_date=?
                                AND (applies_to='All' OR applies_to IS NULL OR applies_to='' OR applies_to=? OR applies_to='Staff' OR applies_to='Associate')""",
...
def recalc_att_day_conn(conn, emp_code, att_date):
    ...
    try:
        ...
        try:
            ...
            is_emp_wo_day = (d_obj.weekday() == get_emp_weekly_off_num(emp_code, conn, on_date=d_obj))
        except:
            is_emp_wo_day = False
        rec = conn.execute(
            "SELECT in_time, out_time, status FROM attendance WHERE emp_code=? AND att_date=?",
    ...
    while True:
        try:
            ...
            try:
                ...
                m = today.month
                emps = conn.execute("SELECT emp_code, category FROM employees WHERE status='Active'").fetchall()
                _el_cap, _cl_cap = get_leave_earn_caps(conn)
                ...
                for lt in earn_types:
                    ...
                    for e in emps:
                        ...
                        eff_earn = earn_days
                        if lt_code == "el":
                            eff_earn = min(eff_earn, leave_earn_room(conn, ec, y, "el", _el_cap))
                        elif lt_code == "cl":
                            eff_earn = min(eff_earn, leave_earn_room(conn, ec, y, "cl", _cl_cap))
                        if eff_earn <= 0:
                            continue
                        ...
                        try:
                            conn.execute(f"""UPDATE leave_balance SET {lt_code}_allotted=ROUND({lt_code}_allotted+?,4)
                                WHERE emp_code=? AND year=?""", (eff_earn, ec, y))
                        except Exception:
